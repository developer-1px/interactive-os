...
function extractImports(filePath: string): string[] {
  ...
  while ((match = IMPORT_RE.exec(content)) !== null) {
    ...
    if (
      ...
      !specifier.startsWith("@os") &&
      !specifier.startsWith("@apps") &&
      !specifier.startsWith("@inspector") &&
      !specifier.startsWith("@/")
    ) {
      continue;
    }
...
function analyze() {
  ...
  for (const stats of folderStats.values()) {
    stats.cohesion =
      stats.totalImports > 0 ? stats.internalImports / stats.totalImports : 1;
  }
...
function printReport({
  ...
  const sorted = [...folderStats.values()].sort((a, b) =>
  ...
  console.log(
    "║              📁 Folder Dependency Health Report                     ║",
  );
  console.log(
    "╚══════════════════════════════════════════════════════════════════════╝",
  );
  ...
  console.log(
    "│  📊 Cohesion Summary (higher = more self-contained)                │",
  );
  console.log(
    "├────────────────────────────┬────────┬──────────┬──────────┬─────────┤",
  );
  console.log(
    "│ Folder                     │ Total  │ Internal │ External │ Cohesn  │",
  );
  console.log(
    "├────────────────────────────┼────────┼──────────┼──────────┼─────────┤",
  );
  ...
  for (const s of sorted) {
    ...
    console.log(
      `│ ${name} │ ${total}  │ ${internal}  │ ${external}  │ ${cohesion} ${bar} │`,
    );
  }
  ...
  console.log(
    "│  🔗 Dependency Flow (folder → imports from)                        │",
  );
  console.log(
    "└─────────────────────────────────────────────────────────────────────┘",
  );
  ...
  for (const s of sorted) {
    ...
    for (const [dep, count] of deps) {
      ...
      console.log(
        `     → ${dep.padEnd(28)} ${String(count).padStart(3)} ${bar}`,
      );
    }
  }
  ...
  console.log(
    "│  🎯 Fan-in (most widely imported folders = shared/core)            │",
  );
  console.log(
    "└─────────────────────────────────────────────────────────────────────┘",
  );
  ...
  for (const s of fanIn) {
    ...
    console.log(
      `  ${s.folder.padEnd(28)} imported by ${String(s.fanIn).padStart(2)} folders ${bar}`,
    );
  ...
  console.log(
    "│  ⚠️  Layer Violations (higher layer importing lower-numbered layer  │",
  );
  console.log(
    "│     is OK; lower importing higher = VIOLATION)                      │",
  );
  console.log(
    "└─────────────────────────────────────────────────────────────────────┘",
  );
  ...
  for (const s of sorted) {
    ...
    for (const [dep, count] of s.importsFrom.entries()) {
      ...
      if (toLayer > fromLayer) {
        console.log(
          `  ❌ ${s.folder} (L${fromLayer}) → ${dep} (L${toLayer})  [${count} imports]`,
        );
        violationCount++;
      }
    }
  ...
  console.log(
    "│     Shows if each folder mainly imports DOWN (good) or UP (bad)     │",
  );
  console.log(
    "└─────────────────────────────────────────────────────────────────────┘",
  );
  ...
  for (const s of osLayers) {
    ...
    console.log(
      `  ${direction} ${s.folder.padEnd(28)} ↓down:${String(downward).padStart(3)}  ↑up:${String(upward).padStart(3)}  ↔other:${String(lateral).padStart(3)}`,
    );
  }
...
function printMermaid(sorted: FolderStats[]) {
  ...
  console.log(
    "│  📐 Mermaid Diagram (paste into mermaid live editor)               │",
  );
  console.log(
    "└─────────────────────────────────────────────────────────────────────┘",
  );
  ...
  if (violations.length > 0) {
    console.log();
    console.log(
      `  style ${[...new Set(violations)].join(",")} fill:#ff6b6b,color:#fff`,
    );
  }
