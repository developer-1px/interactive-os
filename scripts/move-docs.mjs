...
if (args.length < 2) {
  console.error(
    "Usage: node scripts/move-docs.mjs [--dry-run] <target-dir> <file1> [file2] ...",
  );
  process.exit(1);
}
...
for (const file of filesToMove) {
  ...
  if (!file.endsWith(".md")) {
    console.error(`❌ Not markdown: ${file}`);
    process.exit(1);
  }
}
...
for (const file of filesToMove) {
  ...
  console.log(
    `  git mv ${path.relative(projectRoot, oldAbs)} → ${path.relative(projectRoot, newAbs)}`,
  );
  if (!dryRun) {
    try {
      ...
      console.log(`  ⚠️  Not git-tracked, used regular mv`);
    }
  }
...
function parseLink(url, contextDir) {
  ...
  const filePart = hi >= 0 ? url.slice(0, hi) : url;
  const anchor = hi >= 0 ? url.slice(hi) : "";
  if (!filePart) return null;
  if (filePart.startsWith("file://")) {
    return {
      filePath: decodeURIComponent(filePart.replace("file://", "")),
      anchor,
      isFileUrl: true,
    };
  }
  return {
    filePath: path.resolve(contextDir, filePart),
    anchor,
    isFileUrl: false,
  };
}
