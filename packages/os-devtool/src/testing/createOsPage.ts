            ...
            return registeredKeys
                ...
                .map((zId) => {
                    ...
                    return {
                        ...
                        selectedItemId: Object.entries(zoneState?.items ?? {}).find(([, s]) => s?.["aria-selected"])?.[0] ?? null,
    ...
    const result: OsPage = {
        ...
        setConfig(config: Partial<FocusGroupConfig>) {
            ...
            if (zoneId) {
                ...
                if (entry) {
                    ZoneRegistry.register(zoneId, { ...entry, config: mockConfig.current });
                }
            }
        },
                ...
                selection: {
                    get() {
                        const selected = Object.entries(z.items ?? {})
                            .filter(
                                ([, s]) => (s as Record<string, unknown>)?.["aria-selected"] || (s as Record<string, unknown>)?.["aria-checked"],
                            )
                            .map(([itemId]) => itemId);
        ...
        goto(zoneId: string, opts?: GotoOptions) {
            ...
            if (initialSelection || opts?.initial?.expanded || opts?.initial?.values) {
                ...
                os.setState((s: AppState) =>
                    produce(s, (draft) => {
                        ...
                        if (opts?.initial?.values) {
                            for (const [itemId, value] of Object.entries(opts.initial!.values!)) {
                                z.valueNow[itemId] = value;
                            }
                        }
                    }),
