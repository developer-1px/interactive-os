...
function menuClick(
  overrides: Partial<TriggerClickInput> = {},
): TriggerClickInput {
  return {
...
describe("resolveTriggerClick: dialog", () => {
  ...
  it("click when open → OS_OVERLAY_CLOSE (toggle)", () => {
    const cmd = resolveTriggerClick(
      dialogClick({ isTriggerOverlayOpen: true }),
    );
    expect(cmd).not.toBeNull();
    expect(cmd!.type).toBe("OS_OVERLAY_CLOSE");
  });
...
describe("resolveTriggerClick: tooltip", () => {
  ...
  it("click when open → null (tooltip doesn't toggle on click)", () => {
    expect(
      resolveTriggerClick(tooltipClick({ isTriggerOverlayOpen: true })),
    ).toBeNull();
  });
});
