...
import type { ZoneHandle } from "@os-sdk/app/defineApp/types";
...
export function autoDeepClone<T extends { id: string }>(
  ...
  if (Object.hasOwn(item, "fields")) {
    cloned["fields"] = {
      ...((item as Record<string, unknown>)["fields"] as Record<
        string,
        unknown
      >),
    };
  ...
  if (Array.isArray((item as Record<string, unknown>)["children"])) {
    cloned["children"] = (
      (item as Record<string, unknown>)["children"] as Array<{ id: string }>
    ).map((child) => autoDeepClone(child as T, uid(), uid));
  }
  return cloned as T;
}
