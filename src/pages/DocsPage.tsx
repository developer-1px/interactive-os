    ...
    <div className="select-none">
      {node.isFile ? (
        ...
        >
          <FileText
            size={14}
            className={clsx("opacity-70", { "text-indigo-400": node.path })}
          />
...
const MarkdownComponents: any = {
  h1: ({ node, ...props }: any) => (
    <h1
      className="text-4xl font-light tracking-tight mb-8 mt-4 text-transparent bg-clip-text bg-gradient-to-r from-indigo-400 to-cyan-400"
      {...props}
    />
  ),
  h2: ({ node, ...props }: any) => (
    <h2
      className="text-2xl font-semibold text-white mt-12 mb-6 tracking-tight border-b border-white/5 pb-2"
      {...props}
    />
  ),
  h3: ({ node, ...props }: any) => (
    <h3 className="text-xl font-medium text-slate-100 mt-8 mb-4" {...props} />
  ),
  h4: ({ node, ...props }: any) => (
    <h4 className="text-lg font-medium text-indigo-300 mt-6 mb-3" {...props} />
  ),
  p: ({ node, ...props }: any) => (
    <p className="text-slate-300 leading-7 mb-4 font-light" {...props} />
  ),
  ul: ({ node, ...props }: any) => (
    <ul
      className="list-disc list-outside ml-6 mb-4 text-slate-300 marker:text-indigo-500"
      {...props}
    />
  ),
  ol: ({ node, ...props }: any) => (
    <ol
      className="list-decimal list-outside ml-6 mb-4 text-slate-300 marker:text-indigo-500"
      {...props}
    />
  ),
  li: ({ node, ...props }: any) => <li className="mb-2 pl-1" {...props} />,
  strong: ({ node, ...props }: any) => (
    <strong className="font-bold text-white drop-shadow-sm" {...props} />
  ),
  a: ({ node, ...props }: any) => (
    <a
  ...
  blockquote: ({ node, ...props }: any) => (
    <blockquote
      className="border-l-4 border-indigo-500/30 pl-6 py-2 my-6 bg-white/[0.02] text-slate-400 italic rounded-r border-t border-b border-r border-transparent"
      {...props}
    />
  ),
  hr: ({ node, ...props }: any) => (
