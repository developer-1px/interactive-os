...
function useSharedIsolatedEngine() {
  ...
  const store = useMemo(
    () => createCommandStore(registry, initialState),
    [registry],
  );
  ...
  return (
    <div className="p-4 space-y-2">
      ...
      <ul className="space-y-1">
        {state.data.categoryOrder.map((catId: string) => {
          ...
          return (
            <li
              key={category.id}
              className={`flex items-center justify-between p-2 rounded border ${isSelected ? "border-indigo-500 bg-indigo-500/10" : "border-white/5 bg-white/5"}`}
            >
              <span className="text-sm font-mono">
                {isFocused ? "👉" : "  "} {category.text}
              </span>
              <div className="flex gap-1">
                ...
                {isSelected && (
                  <>
                    <button
                      className="px-1 bg-white/10"
                      onClick={() => dispatch({ type: "MOVE_CATEGORY_UP" })}
                    >
                      ▲
                    </button>
                    <button
                      className="px-1 bg-white/10"
                      onClick={() => dispatch({ type: "MOVE_CATEGORY_DOWN" })}
                    >
                      ▼
                    </button>
                  </>
      ...
      <div className="mt-4 p-2 bg-yellow-500/10 text-yellow-500 text-xs rounded">
        <strong>Logic Test:</strong> Clicking Up/Down uses{" "}
        <code>MOVE_CATEGORY_*</code> commands from <code>SIDEBAR_REGISTRY</code>
        .
      </div>
    </div>
  );
      ...
      {visibleTodos.length === 0 ? (
        ...
        <ul className="space-y-1">
          {visibleTodos.map((todo: any) => (
            <li
              key={todo.id}
              className={`flex items-center gap-2 p-2 rounded bg-white/5 border border-white/5 ${todo.completed ? "opacity-50" : ""}`}
            >
              <input
                type="checkbox"
          ...
          <header className="mb-8 flex items-end justify-between">
            <div>
              ...
              <p className="text-slate-500 mt-2 max-w-xl">
                ...
                We "exploded" the Todo App into separate Mock Components. They
                are physically separated but share the same Logic Brain
                (Engine).
              </p>
            </div>
          ...
          <div className="grid grid-cols-2 gap-6 max-w-5xl mx-auto">
            ...
            <Cell title="Architecture Notes" icon={Database}>
              <div className="p-4 text-xs text-slate-400 leading-relaxed space-y-2">
                <p>
                  This grid proves that <strong>Antigravity Commands</strong>{" "}
                  are portable.
                </p>
                <ul className="list-disc pl-4 space-y-1">
                  <li>
                    <strong>MockCategoryBrain</strong> uses{" "}
                    <code>SIDEBAR_REGISTRY</code>
                  </li>
                  <li>
                    <strong>MockTodoBrain</strong> uses{" "}
                    <code>TODO_LIST_REGISTRY</code>
                  </li>
                  <li>
                    They communicate via the <strong>Shared Engine</strong>.
                  </li>
