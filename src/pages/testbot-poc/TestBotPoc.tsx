        ...
        <div className="flex flex-col gap-5 items-center pb-10">
          ...
          <div className="bg-slate-900 rounded-2xl border border-slate-700/50 p-6 shadow-2xl w-[340px]">
            <h2 className="text-sm font-bold text-slate-300 mb-3 flex items-center gap-2">
              ...
              <span className="ml-auto text-[10px] text-slate-600 font-mono">
                loop · followFocus · disallowEmpty
              </span>
            </h2>
            <Zone
              id="rg-zone"
      ...
      <div className="w-[420px] border-l border-slate-700/50 bg-slate-900/50 flex flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-3 space-y-0.5">
          ...
          {logs.map((log, i) => (
            <div
              key={i}
              className={`flex items-start gap-2 px-3 py-1.5 rounded-md text-xs font-mono transition-all ${
                log.result === "pass"
                  ? "text-emerald-400"
                  : log.result === "fail"
            ...
            >
              ...
              {log.action === "assert" && (
                <span
                  className={`shrink-0 ${log.result === "pass" ? "text-emerald-400" : "text-red-400"}`}
                >
                  {log.result === "pass" ? "✓" : "✗"}
                </span>
              )}
