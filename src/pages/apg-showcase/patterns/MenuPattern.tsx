...
export function MenuPattern() {
    ...
    return (
        <div className="bg-white border border-gray-200 rounded-lg p-6 shadow-sm flex flex-col items-center">
            <h3 className="text-lg font-semibold mb-2 w-full text-center">Menubar & Menu</h3>
            <p className="text-sm text-gray-500 mb-6 w-full text-center">
                Try <kbd>Space</kbd> or <kbd>Enter</kbd> to toggle checkboxes and radios.
            </p>
            ...
            <FocusGroup
                ...
                onCheck={(c) => {
                    if (c.focusId?.startsWith("check-")) toggleCheck(c.focusId.replace("check-", ""));
                    if (c.focusId?.startsWith("radio-")) setRadioValue(c.focusId.replace("radio-", ""));
                    return [];
                }}
                onAction={(c) => {
                    if (c.focusId?.startsWith("check-")) toggleCheck(c.focusId.replace("check-", ""));
            ...
            >
                ...
                <div className="px-3 py-1 text-xs font-bold text-gray-400 uppercase tracking-wider">Show</div>
                <FocusItem
                    id="check-ruler"
          ...
          "
                >
                    <span>Ruler</span>
                    <span className="text-indigo-600 opacity-0 group-aria-checked:opacity-100">✓</span>
                </FocusItem>
                <FocusItem
                    id="check-grid"
                    role="menuitemcheckbox"
          ...
          "
                ...
                <div className="px-3 py-1 text-xs font-bold text-gray-400 uppercase tracking-wider">Alignment</div>
