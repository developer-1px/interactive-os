...
export class TestRunner {
    ...
    private async executeSuite(
        ...
        this.emit({ type: "suite:end", name: suite.name, path, status: result.status });
     ...
     */
    async run(moduleLoader: () => Promise<any>, filePath: string): Promise<SuiteResult[]> {
        // Reset state
        this.rootSuites = [];
        this.suiteStack = [];
        this._results = [];
