        ...
        <div className="space-y-1.5">
            ...
            {decoded.type === "button" && (
                <input
                    ...
                    onChange={(e) => {
                        const updated = JSON.stringify({ text, href: e.target.value || undefined });
                        onChange(updated);
                    }}
                    onFocus={onFocus}
                    onBlur={onBlur}
...
const COLOR_MAP: Record<string, string> = {
    ...
    red: "#ef4444", yellow: "#eab308", pink: "#ec4899",
};

function ColorWidget({ value, onChange, onFocus, onBlur }: WidgetProps) {
    const bg = Object.entries(COLOR_MAP).find(([k]) => value.includes(k))?.[1] ?? "#94a3b8";
