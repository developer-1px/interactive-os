...
export function NCPNewsBlock() {
  ...
  const [cards, setCards] = useState([
    ...
    },
    {
      ...
      style: {
        background:
          "linear-gradient(135deg, #ec4899 0%, #8b5cf6 50%, #10b981 100%)",
      },
    },
    {
      id: "news-3",
      ...
      style: {
        background:
          "radial-gradient(circle at 30% 30%, #1e3a8a 0%, #0f172a 100%)",
      },
    },
  ]);

  return (
    <OS.Zone id="ncp-news" role="builderBlock" className="py-20 px-4 bg-white">
          ...
          <div className="flex gap-2">
            <OS.Item id="ncp-news-prev">
              {({ isFocused }: { isFocused: boolean }) => (
                <button
                  className={`w-10 h-10 rounded-full border border-slate-200 flex items-center justify-center text-slate-400 ${isFocused ? "ring-2 ring-slate-400 bg-slate-50" : "hover:bg-slate-50"}`}
                >
                  <ChevronLeft size={20} />
                </button>
              )}
            ...
            <OS.Item id="ncp-news-next">
              {({ isFocused }: { isFocused: boolean }) => (
                <button
                  className={`w-10 h-10 rounded-full border border-slate-900 bg-slate-900 flex items-center justify-center text-white ${isFocused ? "ring-4 ring-slate-300" : "hover:bg-slate-800"}`}
                >
