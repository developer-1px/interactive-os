...
export function TestBox({ title, status, logs, onRun, children, description }: TestBoxProps) {
    ...
    return (
        <div className={`
            ...
            ${status === 'running' ? 'border-amber-400' :
                status === 'pass' ? 'border-emerald-400' :
        ...
        `}>
            ...
            <div className="bg-gray-50 p-2 flex items-center justify-between border-b border-gray-100">
                <div className="flex items-center gap-2">
                    ...
                    <h3 className="font-bold text-xs text-gray-700 uppercase tracking-wider">{title}</h3>
                </div>
                <div className="flex gap-2">
                    <button
                        onClick={() => setShowDocs(!showDocs)}
                        className={`text-[10px] px-2 py-0.5 rounded border ${showDocs ? 'text-gray-700 border-gray-300 bg-gray-100' : 'border-transparent text-gray-400'
                            }`}
                    >
                        Docs
                    </button>
                    <button
                        ...
                        className={`text-[10px] px-2 py-0.5 rounded border ${logs.length > 0 ? 'border-gray-300 text-gray-600 hover:bg-gray-100' : 'border-transparent text-gray-400'
                            }`}
            ...
            {showLogs && logs.length > 0 && (
                <div className="bg-gray-100 p-2 border-b border-gray-200 max-h-32 overflow-y-auto">
                    <div className="space-y-1 font-mono text-[10px]">
                        {logs.map((log, i) => (
                            <div key={i} className={`truncate ${log.startsWith('✅') ? 'text-emerald-600' :
                                log.startsWith('❌') ? 'text-red-600' :
                                    log.startsWith('→') ? 'text-blue-600' : 'text-gray-500'
                                }`}>
                                {log}
                            </div>
                        ))}
