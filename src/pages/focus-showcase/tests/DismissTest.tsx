...
export function DismissTest() {
    ...
    const runTest = async () => {
        ...
        try {
            ...
            click('#dis-esc-1');
            await wait(200);
            const isSelected = document.querySelector('#dis-esc-1')?.getAttribute('aria-selected') === 'true';
            assert(isSelected, 'Item Selected after click', localLogs);

            press('Escape');
            await wait(200);
            const isDeselected = document.querySelector('#dis-esc-1')?.getAttribute('aria-selected') !== 'true';
    ...
    const description = (
        <div className="space-y-2">
            <p>
                <strong>Dismiss</strong> strategies handle how a group reacts to Escape or Outside Clicks.
            </p>
    ...
    return (
        <TestBox title="Dismiss / Escape" status={status} logs={logs} onRun={runTest} description={description}>
            <div className="flex flex-col gap-6">
                {/* Deselect */}
                <div className="space-y-2">
                    <div className="text-[10px] font-mono text-gray-500 uppercase">Escape = Deselect</div>
                    <FocusGroup
                        id="dis-esc"
                        role="listbox"
                    ...
                    >
                        {['Item 1', 'Item 2'].map(item => (
                            <FocusItem key={item} id={`dis-esc-${item.split(' ')[1]}`} role="option" className="px-3 py-1.5 rounded hover:bg-gray-100 aria-[selected=true]:bg-indigo-100 aria-[selected=true]:text-indigo-700 text-sm transition-all border border-transparent aria-[selected=true]:border-indigo-300">
                                {item}
                            </FocusItem>
                        ))}
                    </FocusGroup>
                    <div className="text-[10px] text-gray-500">Select an item, then press Escape to deselect.</div>
                </div>

                {/* Close */}
                <div className="space-y-2">
                    <div className="text-[10px] font-mono text-gray-500 uppercase">Escape = Close (Menu)</div>
                    <FocusGroup
                        id="dis-close"
                        role="menu"
                        navigate={{ orientation: 'vertical' }}
                    ...
                    >
                        {['Menu Item A', 'Menu Item B'].map(item => (
                            <FocusItem key={item} id={`dis-close-${item.split(' ')[2]}`} role="menuitem" className="px-3 py-1.5 rounded hover:bg-gray-100 aria-[current=true]:bg-violet-100 aria-[current=true]:text-violet-700 text-sm transition-all border border-transparent aria-[current=true]:border-violet-300">
                                {item}
                            </FocusItem>
                        ))}
                    </FocusGroup>
