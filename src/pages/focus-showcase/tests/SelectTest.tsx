...
export function SelectTest() {
    ...
    const runTest = async () => {
        ...
        try {
            ...
            click('#sel-toggle-1');
            await wait(20);
            assert(document.querySelector('#sel-toggle-1')?.getAttribute('aria-selected') === 'true', 'Click selects', localLogs);
            click('#sel-toggle-1', { ctrl: true });
            await wait(20);
            assert(document.querySelector('#sel-toggle-1')?.getAttribute('aria-selected') !== 'true', 'Ctrl+Click deselects', localLogs);

            // Test 2: Range Selection
            click('#sel-range-0');
            await wait(20);
            click('#sel-range-2', { shift: true });
            await wait(20);
            assert(document.querySelector('#sel-range-1')?.getAttribute('aria-selected') === 'true', 'Shift+Click selects range (middle item)', localLogs);

            // Test 3: Follow Focus (Radio)
            click('#sel-radio-a');
            await wait(20);
            localLogs.push('→ Navigating Down in Radio Group');
            click('#sel-radio-b');
            await wait(20);
            assert(document.querySelector('#sel-radio-b')?.getAttribute('aria-selected') === 'true', 'Focus change updates selection (Follow Focus)', localLogs);
    ...
    const description = (
        <div className="space-y-2">
            <p>
                <strong>Selection Modes</strong> determine how users interact with items.
            </p>
            <ul className="list-disc list-inside space-y-1 text-gray-500">
                <li><code className="text-gray-700 bg-gray-100 px-1 rounded">mode: 'single'</code>: Standard Radio/Tab behavior.</li>
                <li><code className="text-gray-700 bg-gray-100 px-1 rounded">mode: 'multiple'</code>: Checkbox/Grid behavior.</li>
                <li><code className="text-gray-700 bg-gray-100 px-1 rounded">toggle: true</code>: Allows deselecting (Ctrl+Click).</li>
                <li><code className="text-gray-700 bg-gray-100 px-1 rounded">range: true</code>: Shift+Click range selection.</li>
                <li><code className="text-gray-700 bg-gray-100 px-1 rounded">followFocus: true</code>: Selection follows focus (Tabs/Radio).</li>
            </ul>
        </div>
    );

    return (
        <TestBox title="Selection Strategies" status={status} logs={logs} onRun={runTest} description={description}>
            <div className="flex flex-col gap-6">
                ...
                <div className="space-y-2">
                    ...
                    <div className="text-[10px] text-gray-500">Try Ctrl+Click or Shift+Click</div>
                </div>
                    ...
                    <FocusGroup id="sel-radio" role="radiogroup" navigate={{ orientation: 'vertical' }} select={{ mode: 'single', followFocus: true, disallowEmpty: true }} className="flex flex-col bg-gray-50 p-2 rounded border border-gray-200 gap-1">
                        {['A', 'B'].map(opt => (
                            <FocusItem key={opt} id={`sel-radio-${opt.toLowerCase()}`} role="radio" className="px-3 py-1.5 flex items-center gap-3 rounded hover:bg-gray-100 transition-colors group data-[focused=true]:ring-2 data-[focused=true]:ring-blue-500 data-[focused=true]:ring-inset">
                                ...
                                <span className="text-sm text-gray-600 group-aria-[selected=true]:text-rose-600">{opt}</span>
                            </FocusItem>
                        ))}
                    </FocusGroup>
                </div>
