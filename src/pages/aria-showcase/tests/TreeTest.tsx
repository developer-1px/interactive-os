...
export function defineTreeTests(bot: TestBot) {
  bot.describe("Tree: Expand/Collapse", async (t) => {
    ...
    await t.expect("#tree-src").toBeFocused();

    // 2. Press ArrowRight to expand
    await t.press("ArrowRight");
    await t.expect("#tree-src").toHaveAttribute("aria-expanded", "true");

    // 3. ArrowDown to navigate to first child
    await t.press("ArrowDown");
    await t.expect("#tree-components").toBeFocused();

    // 4. ArrowDown to second child
    await t.press("ArrowDown");
    await t.expect("#tree-app").toBeFocused();
    ...
    await t.expect("#tree-src").toHaveAttribute("aria-expanded", "false");
  });
  ...
  bot.describe("Tree: Nested Navigation", async (t) => {
    ...
    await t.press("ArrowDown");
    await t.expect("#tree-index").toBeFocused();

    // 3. Navigate to public folder
    await t.press("ArrowDown");
    await t.expect("#tree-public").toBeFocused();

    // 4. Expand public folder
    await t.press("ArrowRight");
    await t.expect("#tree-public").toHaveAttribute("aria-expanded", "true");
  });
