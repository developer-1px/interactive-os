...
export const todoPhysicsMiddleware = (
  ...
  if (request) {
    ...
    if (request === "PREV") {
      ...
      if (currentFocus === "DRAFT" || !currentFocus) {
        ...
        const index = visible.indexOf(Number(currentFocus));
        if (index <= 0) targetId = "DRAFT";
        else targetId = String(visible[index - 1]);
      }
      ...
      if (currentFocus === "DRAFT" || !currentFocus) {
        ...
        if (index === visible.length - 1) targetId = "DRAFT";
        else if (index !== -1) targetId = String(visible[index + 1]);
      ...
      const currentId = currentFocus
        ? String(currentFocus)
        : rawNewState.ui.selectedCategoryId;
      const index = order.indexOf(currentId);
      if (index > 0) targetId = order[index - 1];
      ...
      const currentId = currentFocus
        ...
        : rawNewState.ui.selectedCategoryId;
      const index = order.indexOf(currentId);
      if (index !== -1 && index < order.length - 1) targetId = order[index + 1];
    } else {
      // Direct ID Request
