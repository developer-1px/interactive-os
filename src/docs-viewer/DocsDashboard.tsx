...
export function DocsDashboard({ allFiles, onSelect }: DocsDashboardProps) {
    ...
    const projectFolders = Array.from(
        ...
        .map((name) => ({
            ...
            status: ["On Track", "At Risk", "Completed"][Math.floor(Math.random() * 3)],
            lastUpdated: new Date(Date.now() - Math.floor(Math.random() * 1000000000)).toLocaleDateString(),
        }))
        .slice(0, 6);
    ...
    const recentFiles = [...allFiles]
        ...
        .map(f => ({
            ...f,
            editedAt: new Date(Date.now() - Math.floor(Math.random() * 5 * 24 * 60 * 60 * 1000)), // Random time within 5 days
        }));
    ...
    return (
        <div className="p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
            ...
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-slate-900 tracking-tight">{greeting}, User</h1>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-6">
                ...
                <div className="col-span-1 md:col-span-4 flex flex-col bg-slate-50 rounded-2xl p-6 border border-slate-100 shadow-sm relative overflow-hidden group hover:shadow-md transition-all">
                    <div className="flex items-center justify-between mb-4">
                        <div className="flex items-center gap-2 text-slate-800 font-semibold">
                            ...
                            <span>Inbox</span>
                            <span className="bg-indigo-100 text-indigo-700 text-xs px-2 py-0.5 rounded-full">{inboxItems.length}</span>
                        </div>
                        <ArrowRight size={16} className="text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer" onClick={() => onSelect("0-inbox")} />
                    </div>
                ...
                <div className="col-span-1 md:col-span-8 bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <button onClick={() => onSelect("1-project")} className="text-xs text-slate-500 hover:text-slate-900 font-medium">View All</button>
                    </div>

                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                        {projectFolders.map((project) => (
                            <div key={project.path} className="p-4 rounded-xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-emerald-200 hover:shadow-md transition-all cursor-pointer group" onClick={() => onSelect(project.path)}>
                                <div className="flex justify-between items-start mb-3">
                                    <div className={clsx("w-2 h-2 rounded-full mt-1.5",
                                        project.status === "On Track" ? "bg-emerald-500" :
                                            project.status === "At Risk" ? "bg-rose-500" : "bg-slate-300"
                                    )} />
                                    <MoreHorizontal size={14} className="text-slate-300 hover:text-slate-600" />
                                </div>
                                <h3 className="font-semibold text-slate-800 text-sm mb-1 group-hover:text-emerald-700 truncate">{project.name}</h3>
                                <p className="text-xs text-slate-500 mb-3">Updated {project.lastUpdated}</p>
                ...
                <div className="col-span-1 md:col-span-12 bg-white rounded-2xl p-6 border border-slate-200 shadow-sm">
                    ...
                    <div className="divide-y divide-slate-100">
                        {recentFiles.map((file, i) => (
                            <div key={file.path} className="flex items-center justify-between py-3 hover:bg-slate-50 px-2 rounded-lg -mx-2 transition-colors cursor-pointer group" onClick={() => onSelect(file.path)}>
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <h4 className="text-sm font-medium text-slate-900 group-hover:text-indigo-600 transition-colors">{cleanLabel(file.name)}</h4>
                                        <p className="text-xs text-slate-500 flex items-center gap-1">
                                            <span className="text-slate-400">{file.path.split('/')[0]}</span>
                                            <span>•</span>
                                ...
                                <div className="text-xs text-slate-400 font-medium">
                                    {file.editedAt.toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </div>
                            </div>
                        ))}
                    </div>
