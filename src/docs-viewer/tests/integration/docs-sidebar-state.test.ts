...
import { toggleSection } from "@/docs-viewer/app";
...
describe("T4: DocsSidebar — isOpen via OS command (not useState)", () => {
    ...
    beforeEach(() => {
        ...
        appGroup.defineCommand(
            "DOCS_TOGGLE_SECTION",
            (ctx: { readonly state: any }) =>
                (payload: { sectionId: string }) => ({
                    state: produce(ctx.state, (draft: any) => {
                        if (!draft.docsViewer) draft.docsViewer = { ui: { sections: { recent: { isOpen: true }, favorites: { isOpen: true } } } };
    ...
    it("#2 dispatching DOCS_TOGGLE_SECTION changes state to collapsed", () => {
        page.kernel.dispatch(
            toggleSection({ sectionId: "recent" }),
        );
