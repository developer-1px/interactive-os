...
function makeDOMRect(
    ...
    return { top, left, width, height, x: left, y: top, bottom: top + height, right: left + width, toJSON() { } } as DOMRect;
}
...
describe("useElementRect", () => {
    ...
    it("returns null → rect on element change", () => {
        ...
        rerender({ el });
        expect(result.current).toEqual({ top: 10, left: 20, width: 100, height: 50 });
    });
