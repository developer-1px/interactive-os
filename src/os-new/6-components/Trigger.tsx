...
function TriggerPortal({
  ...
  const isOpen = kernel.useComputed(
    (s) => s.os.overlays.stack.some((e) => e.id === overlayId),
  );
  ...
  return (
    <OverlayContext.Provider value={{ overlayId }}>
      ...
      >
        <div className={`os-modal-content${contentClassName ? ` ${contentClassName}` : ""}`}>
          {title && (
