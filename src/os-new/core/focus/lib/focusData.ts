...
/** Minimal store interface for zone state (Zustand-compatible). */
export interface ZoneStore {
  getState(): ZoneStoreState;
  setState(partial: Partial<ZoneStoreState>): void;
...
export const FocusData = {
  ...
  popAndRestoreFocus(): boolean {
    ...
    setTimeout(() => {
      ...
      // Dispatch through kernel FOCUS command (updates kernel state → FocusGroup bridge → Zustand → aria-current)
      Promise.all([
        import("@/os-new/3-commands/focus"),
        import("@/os-new/kernel"),
      ]).then(([{ FOCUS }, { kernel }]) => {
        kernel.dispatch(FOCUS({ zoneId: entry.zoneId, itemId: targetItemId }));
      });
    }, 50);
