...
test("debug quickpick zone state", async ({ page }) => {
    ...
    const input = dialog.locator('input[placeholder="Search routes and docs..."]');
    ...
    const afterArrow = await page.evaluate(() => {
        ...
        return Array.from(options).slice(0, 5).map(el => ({
    ...
    console.log("Active zones (aria-current=true):", JSON.stringify(kernelFocus, null, 2));
