...
import { Search, Layers, ListTree, ListMinus, Copy, Download, Trash2, Settings, MousePointerClick, MousePointer2, Eye, Keyboard } from "lucide-react";
import type { Transaction } from "@kernel/core/transaction";
import { inferSignal } from "../../utils/inferSignal";
import { CollapsibleSection } from "./CollapsibleSection";
import { TransactionItem } from "./TransactionItem";
import { formatTime, copyAllToClipboard } from "../../utils/inspectorFormatters";
...
export type DisplayEntry =
    ...
    | { kind: "group"; txs: Transaction[]; representative: Transaction; count: number; filteredIndex: number };
...
export function groupConsecutive(
    ...
    for (let i = 1; i < txs.length; i++) {
        ...
        if ((isConsecutiveFocus || isConsecutiveKey || isConsecutiveMouse) && isFast) {
...
export function TimelineGroup({
    ...
    const icon =
        trigger.kind === "MOUSE" ? (
            trigger.raw === "Click" ? (
                <MousePointerClick size={12} className="text-[#94a3b8]" strokeWidth={2.5} />
            ) : (
                        ...
                        displayEntries.map((entry, i) => {
                            if (entry.kind === "group") {
                                ...
                                const prevTx = prevEntry
                                    ...
                                    : undefined;
                                const timeDeltaMs = prevTx ? firstTx.timestamp - prevTx.timestamp : 0;
                                const totalDurationMs =
                                    entry.txs[entry.txs.length - 1]!.timestamp - firstTx.timestamp;
