...
const ITEM_CLS = `px-2 py-1 rounded text-xs cursor-pointer select-none border
  bg-white text-slate-600 border-slate-200 hover:bg-slate-50
  data-[focused=true]:bg-blue-50 data-[focused=true]:text-blue-700
  data-[focused=true]:border-blue-300`;

const TOOL_CLS = `px-2 py-1 rounded text-xs font-bold cursor-pointer select-none border
  bg-white text-slate-600 border-slate-200 hover:bg-slate-50
  data-[focused=true]:bg-violet-50 data-[focused=true]:text-violet-700
  data-[focused=true]:border-violet-300`;

const RADIO_CLS = `px-2 py-1 rounded text-xs cursor-pointer select-none border
...
const CELL_CLS = `aspect-square flex items-center justify-center rounded text-xs font-bold
...
export function TestBotPanel() {
  ...
  const createPage = useCallback(() => {
    const container: HTMLElement | null = isPageMode
      ? document.body
  ...
  const handleQuickRun = useCallback(async () => {
    ...
    const container: HTMLElement = isPageMode
      ? document.body
  ...
  return (
    <div className="flex-1 flex flex-col overflow-hidden bg-slate-50 font-sans">
      ...
      {!isPageMode && (
        ...
        >
          <p className="text-[9px] font-mono text-slate-400 uppercase mb-1.5 tracking-wider">
            Test Subjects
          </p>
            ...
            <div className="flex gap-2">
              ...
              >
                {[
                  ...
                  <Item key={id} id={id} className={TOOL_CLS}>
                    {l}
                  </Item>
                ))}
              </Zone>
              ...
              >
                {[
                  ...
                  <Item key={id} id={id} className={RADIO_CLS}>
                    {l}
                  </Item>
                ))}
              </Zone>
