...
/** Callbacks invoked on every goto() — for pages with independent kernels. */
const gotoResetCallbacks = new Set<() => void>();

/** Register a reset callback to be invoked on every page.goto(). */
export function registerGotoReset(cb: () => void): () => void {
  gotoResetCallbacks.add(cb);
  return () => gotoResetCallbacks.delete(cb);
}
...
export class ShimPage implements Page {
  ...
  async goto(_url: string) {
    ...
    resetAllAppSlices();
    // Invoke custom reset callbacks (independent kernels, etc.)
    for (const cb of gotoResetCallbacks) cb();
...
export class ShimLocator implements Locator {
  ...
  /** Resolve all DOM elements matching this locator. */
  private resolveAllElements(): Element[] {
    ...
    if ("role" in this.selector && this.selector.role) {
      ...
      for (const el of scope.querySelectorAll("*")) {
        if (!matchesRole(el, this.selector.role!)) continue;
        if (this.selector.name && !matchesName(el, this.selector.name))
          continue;
        results.push(el);
      }
      return results;
  ...
  first(): ShimLocator {
    const els = this.resolveAllElements();
    if (els.length === 0) return new ShimLocator(this.t, "__shim_empty__");
    return new ShimLocator(this.t, getUniqueSelector(els[0]!));
  }

  last(): ShimLocator {
    const els = this.resolveAllElements();
    if (els.length === 0) return new ShimLocator(this.t, "__shim_empty__");
    return new ShimLocator(this.t, getUniqueSelector(els[els.length - 1]!));
  }

  nth(index: number): ShimLocator {
    const els = this.resolveAllElements();
    if (index < 0 || index >= els.length)
      return new ShimLocator(this.t, "__shim_empty__");
    return new ShimLocator(this.t, getUniqueSelector(els[index]!));
  }

  async count(): Promise<number> {
    return this.resolveAllElements().length;
  }

  async textContent(): Promise<string | null> {
    const el = resolveElement(this.selector);
  ...
  async allTextContents(): Promise<string[]> {
    return this.resolveAllElements().map((el) => el.textContent ?? "");
  }
}
