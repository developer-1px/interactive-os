    ...
    return (
      ...
      >
        <div className="flex items-center gap-2 min-w-0 overflow-hidden">
          <span
            className={`w-1.5 h-1.5 rounded-full flex-shrink-0 transition-colors ${isLastExecuted ? "bg-white shadow-[0_0_5px_rgba(255,255,255,0.8)]" : !isDisabled ? "bg-indigo-500" : "bg-slate-700"}`}
          />
          <span
          ...
          >
            {cmd.when && (
              <div
                ...
                className={`${isLastExecuted ? "text-indigo-200" : "text-slate-500 hover:text-indigo-400"} transition-colors`}
              >
                <GitGraph size={9} />
              </div>
            )}
  ...
  ({
    ...
    return (
      <section className="px-3 py-2 border-b border-white/5 bg-white/[0.01]">
        <div className="flex items-center justify-between mb-2">
          <h3 className="text-[9px] font-black text-slate-500 uppercase tracking-widest flex items-center gap-2">
            <span className="w-1 h-3 bg-emerald-500/40 rounded-full" /> Command
            Registry (via Menu)
          </h3>
          ...
          <div key={i} className="group relative pl-3.5">
            ...
            <div className="text-[8px] text-slate-500 pl-1 border-l-2 border-transparent group-hover:border-white/10 transition-all truncate font-mono">
              ...
              <span className="text-emerald-500/60">
                {String(entry.resultingState.focusId)}
              </span>
            </div>
          </div>
        ))}
