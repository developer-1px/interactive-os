...
import { AddTodo, ToggleTodo, DeleteTodo, SyncEditDraft, UpdateTodoText, CancelEdit, StartEdit, SyncDraft, ToggleView } from '../lib/todoCommands';
import { Plus, Check, CornerDownLeft, Trash2, Sparkles, Loader2, Kanban, List } from 'lucide-react';
                                    ...
                                    return (
                                        ...
                                        >
                                            <Trigger command={ToggleTodo({ id: todo.id })}>
                                                <div className={`w-5 h-5 rounded-lg border-2 flex items-center justify-center transition-all duration-300 cursor-pointer flex-shrink-0 ${todo.completed
                                                    ? 'bg-indigo-500 border-indigo-500 shadow-[0_0_10px_rgba(99,102,241,0.4)] scale-110'
                                                    : 'border-slate-600 bg-transparent'
                                                    }`}>
                                                    {todo.completed && <Check size={12} className="text-white" strokeWidth={4} />}
                                                </div>
                                            </Trigger>
                                            ...
                                            <div className="flex-1 min-w-0">
                                                {isEditing ? (
                                                    ...
                                                    <span
                                                        className={`block truncate text-lg transition-all duration-300 pointer-events-none ${todo.completed ? 'text-slate-500 line-through decoration-slate-600' : 'text-slate-200 font-medium'
                                                            }`}
                                                    >
                                                        {todo.text}
                                                    </span>
