...
export const MockIcon: React.FC<MockIconProps> = ({
    ...
    return (
        <div
            ...
            aria-hidden="true"
        >
            {IconComponent ? <IconComponent size={size} color={color} /> : <span>?</span>}
        </div>
    );
};
...
export const MockText: React.FC<MockTextProps> = ({
    ...
    const ComputedComponent = variant === 'h1' ? 'h1' : variant === 'h2' ? 'h2' : 'p';
...
export const MockShortcut: React.FC<MockShortcutProps> = ({ keys, className }) => {
    return (
        <div className={`flex items-center gap-1 ${className || ''}`} data-testid="mock-shortcut">
            {keys.map((k, i) => (
