        ...
        <div className="flex-1 flex overflow-hidden">
            <div className="flex-1 flex h-full gap-6 overflow-x-auto p-8 custom-scrollbar items-start">
                {categoryOrder.map((catId, index) => {
                    ...
                    const prevZone = index > 0 ? `board_col_${categoryOrder[index - 1]}` : 'sidebar';
                    const nextZone = index < categoryOrder.length - 1 ? `board_col_${categoryOrder[index + 1]}` : undefined;
                            ...
                            >
                                ...
                                <Item id={`board_col_${catId}_header`} className="outline-none">
                                    <div className="p-4 border-b border-white/5 flex justify-between items-center cursor-pointer hover:bg-white/5 transition-colors rounded-t-xl">
                                        <h3 className={`font-medium text-sm tracking-wide ${activeColumn ? 'text-indigo-300' : 'text-slate-400'}`}>
                                            {category.text}
                                        </h3>
                                            ...
                                            >
                                                <div className="flex items-start gap-3">
                                                    <Trigger command={ToggleTodo({ id: todo.id })}>
                                                        <div className={`w-4 h-4 mt-1 rounded border flex items-center justify-center transition-all cursor-pointer flex-shrink-0 
                                                        ${isCompleted
                                                    ...
                                                    <span className={`text-sm leading-snug flex-1 ${isCompleted ? 'text-slate-500 line-through' : 'text-slate-200'}`}>
                                                        {todo.text}
                                                    </span>
                                                </div>
