...
import { isClickExpandable, resolveMouse, type MouseInput } from "@os/1-listeners/mouse/resolveMouse";
...
function baseInput(overrides: Partial<MouseInput> = {}): MouseInput {
    return {
        ...
        isLabel: false, labelTargetItemId: null, labelTargetGroupId: null,
...
describe("Tree Expand: keyboard (ArrowRight/Left)", () => {
    function treeFactory(focusedItem = "folder:api") {
        ...
        page.setItems(["folder:api", "api/auth", "api/users", "folder:docs", "docs/readme"]);
        page.setExpandableItems(["folder:api", "folder:docs"]);
        page.setTreeLevels({
            "folder:api": 1, "api/auth": 2, "api/users": 2,
            "folder:docs": 1, "docs/readme": 2,
        });
        page.setRole("test-tree", "tree", { onAction });
        page.setConfig({
            navigate: { orientation: "vertical", loop: false, seamless: false, typeahead: false, entry: "first", recovery: "next", arrowExpand: true },
            select: { mode: "single", followFocus: false, disallowEmpty: false, range: false, toggle: true },
        });
