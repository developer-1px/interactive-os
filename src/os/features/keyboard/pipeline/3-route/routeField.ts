...
export function routeField(intent: KeyboardIntent): boolean {
  ...
  logger.debug("KEYBOARD", "[P3:Route] Field:", {
    canonicalKey,
    fieldId,
    mode,
    isEditing,
  ...
  if (intent.isComposing) {
    if (
      ["ArrowLeft", "ArrowRight", "ArrowUp", "ArrowDown"].includes(canonicalKey)
    ) {
  ...
  if (mode === "deferred") {
    ...
    if (canonicalKey === "Enter" && !multiline) {
      if (!isEditing) {
        dispatch?.({
          type: OS_COMMANDS.FIELD_START_EDIT,
          payload: { fieldId },
        });
      } else {
  ...
  if (
    multiline &&
    (canonicalKey === "ArrowUp" || canonicalKey === "ArrowDown")
  ) {
