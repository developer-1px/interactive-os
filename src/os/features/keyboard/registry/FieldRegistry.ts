...
export const useFieldRegistry = create<FieldRegistryState & FieldRegistryActions>((set, get) => ({
    fields: new Map(),
    activeFieldId: null,
    ...
    setEditing: (id, isEditing) => {
        set((state) => {
            ...
            return {
                fields: newFields,
                activeFieldId: isEditing ? id : (state.activeFieldId === id ? null : state.activeFieldId)
            };
        });
    },
...
export const FieldRegistry = {
    ...
    register: (id: string, config: FieldConfig) => useFieldRegistry.getState().register(id, config),
    unregister: (id: string) => useFieldRegistry.getState().unregister(id),
    setEditing: (id: string, isEditing: boolean) => useFieldRegistry.getState().setEditing(id, isEditing),
    updateValue: (id: string, value: string) => useFieldRegistry.getState().updateValue(id, value),
