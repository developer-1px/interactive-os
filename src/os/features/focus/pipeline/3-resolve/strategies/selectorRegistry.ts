...
const resolveSingle: SelectStrategy = (targetId, currentSelection) => {
  if (!targetId)
    return { changed: false, selection: currentSelection, anchor: null };
...
const resolveToggle: SelectStrategy = (
  targetId,
  currentSelection,
  currentAnchor,
  _items,
...
const resolveRange: SelectStrategy = (
  targetId,
  currentSelection,
  currentAnchor,
  items,
...
const resolveNone: SelectStrategy = (
  _target,
  currentSelection,
  currentAnchor,
) => {
  if (currentSelection.length === 0)
    return { changed: false, selection: [], anchor: currentAnchor };
  return {
    changed: true,
    selection: [],
    anchor: null,
...
function registerSelectionStrategy(
  name: string,
