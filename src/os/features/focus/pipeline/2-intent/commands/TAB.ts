...
function handleTrap(ctx: OSContext, direction: 'forward' | 'backward'): OSResult | null {
    const items = ctx.dom.items;
    if (items.length === 0) return null;

    const currentIndex = ctx.focusedItemId ? items.indexOf(ctx.focusedItemId) : -1;
...
function handleEscape(ctx: OSContext, direction: 'forward' | 'backward'): OSResult | null {
    // Escape: 바로 다음/이전 Zone으로 이동
    const siblingZoneId = direction === 'forward'
    ...
    if (siblingZoneId) {
        ...
        const targetItemId = siblingStore?.getState().focusedItemId
...
function handleFlow(ctx: OSContext, direction: 'forward' | 'backward'): OSResult | null {
    const items = ctx.dom.items;
    if (items.length === 0) return null;
