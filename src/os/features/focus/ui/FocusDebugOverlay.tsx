...
export function FocusDebugOverlay() {
  ...
  const measure = useCallback(() => {
    ...
    const zoneId = FocusData.getActiveZoneId();
    const zoneData = zoneId ? FocusData.getById(zoneId) : null;
    const itemId = zoneData?.store.getState().focusedItemId ?? null;
  ...
  useEffect(() => {
    ...
    // ── Observers: track resize & DOM mutations ──
    const setupObservers = () => {
      ...
      const zoneId = FocusData.getActiveZoneId();
      const zoneData = zoneId ? FocusData.getById(zoneId) : null;
      const itemId = zoneData?.store.getState().focusedItemId;
      const el = itemId ? document.getElementById(itemId) : null;
      if (el) {
        ro.observe(el);
        prevEl = el;
    ...
    const onFocusChange = () => {
      // Re-subscribe to new zone's store
      unsubStore?.();
      unsubStore = null;
      ...
      const zoneData = zoneId ? FocusData.getById(zoneId) : null;
      if (zoneData) {
        unsubStore = zoneData.store.subscribe(() => {
          // Re-attach ResizeObserver to new focused element
          const newItemId = zoneData.store.getState().focusedItemId;
    ...
    const unsubZone = FocusData.subscribeActiveZone(onFocusChange);
