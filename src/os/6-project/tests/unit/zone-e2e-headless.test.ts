...
import { createOsPage } from "@os/app/defineApp/page";
...
describe("Zone E2E → Headless (Playwright interface)", () => {
  test("Entry: click transfers aria-current exclusively", () => {
    ...
    page.locator("nav-apple").click();
    expect(
      page.locator("nav-apple").toHaveAttribute("aria-current", "true"),
    ).toBe(true);

    // Playwright: await page.locator("#nav-banana").click();
    page.locator("nav-banana").click();
    expect(
      page.locator("nav-banana").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
    expect(
      page.locator("nav-apple").toHaveAttribute("aria-current", "true"),
    ).toBe(false);
  ...
  test("Navigate: Vertical Loop — wraps at boundaries", () => {
    ...
    expect(
      page.locator("nav-cherry").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
    expect(
      page.locator("nav-apple").toHaveAttribute("aria-current", "true"),
    ).toBe(false);
  ...
  test("Navigate: Horizontal Clamped — stops at boundaries", () => {
    ...
    expect(
      page.locator("nav-bold").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
    ...
    expect(
      page.locator("nav-italic").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
    expect(
      page.locator("nav-bold").toHaveAttribute("aria-current", "true"),
    ).toBe(false);
    ...
    page.keyboard.press("ArrowRight");
    expect(
      page.locator("nav-underline").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
  ...
  test("Zone container: getAttribute for role, data-zone", () => {
    ...
    expect(
      page.locator("nav-list").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
  ...
  test("Select: Multi — locator.click with modifiers", () => {
    ...
    expect(page.locator("sel-0").toHaveAttribute("aria-selected", true)).toBe(
      true,
    );

    // Playwright: await page.locator("#sel-2").click({ modifiers: ["Meta"] });
    page.locator("sel-2").click({ modifiers: ["Meta"] });
    expect(page.locator("sel-2").toHaveAttribute("aria-selected", true)).toBe(
      true,
    );
    expect(page.locator("sel-0").toHaveAttribute("aria-selected", true)).toBe(
      true,
    ...
    expect(
      page.locator("sel-zone").toHaveAttribute("aria-multiselectable", true),
    ).toBe(true);
