...
import { computeItem, type ItemState } from "@os/core/library/headless/compute";
import {
  cloneElement,
  forwardRef,
  isValidElement,
...
import { DEFAULT_CONFIG } from "../core/schema/types";
import { useZoneContext } from "./Zone.tsx";

// Re-export for consumers
export type { ItemState } from "@os/core/library/headless/compute";
...
const ItemBase = forwardRef<HTMLElement, ItemProps>(function Item(
  {
    id,
    disabled,
    selected,
  ...
  if (mergeEl) {
    return cloneElement(mergeEl, {
      ...
      ref: combinedRef,
      className:
        [mergeEl.props.className, className].filter(Boolean).join(" ") ||
        undefined,
