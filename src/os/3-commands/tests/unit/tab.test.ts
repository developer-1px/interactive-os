...
describe("resolveTab — top-level orchestrator (SPEC §3.3)", () => {
    ...
    describe("flow", () => {
        ...
        it("returns null at boundary with no next zone", () => {
            expect(resolveTab("c", items, "flow", "forward", "z1", [zones[0]!])).toBeNull();
        });
    });
    ...
    describe("escape", () => {
        ...
        it("immediately escapes to prev zone (backward)", () => {
            const z2Items = ["d", "e", "f"];
            expect(resolveTab("d", z2Items, "escape", "backward", "z2", zones)).toEqual({
                type: "escape",
                zoneId: "z1",
                itemId: "c",
            });
        ...
        it("returns null when no zone to escape to", () => {
            expect(resolveTab("c", items, "escape", "forward", "z1", [zones[0]!])).toBeNull();
        });
    });
