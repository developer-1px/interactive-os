import type { LogicNode, ContextState } from "./types";
...
export const Expect = <T = any>(key: keyof T & string) => {
    const ex = {
        toBe: (v: any) => wrap(c => c[key] === v, `${key} == ${JSON.stringify(v)}`),
        ...
        toBeLessThanKey: (k2: string) => wrap(c => (c[key] as any) < (c[k2] as any), `${key} < ${k2}`),
    };
    return {
        ...ex,
        ...
        not: { toBe: (v: any) => wrap(c => c[key] !== v, `${key} != ${JSON.stringify(v)}`) }
    };
};

export const Rule = {
    and: (...fns: any[]): LogicNode => wrap(
        ctx => fns.every(f => evalContext(f, ctx)),
        `(${fns.join(" && ")})`
    ),
    or: (...fns: any[]): LogicNode => wrap(
        ctx => fns.some(f => evalContext(f, ctx)),
        `(${fns.join(" || ")})`
    ),
};

export const and = Rule.and;
export const or = Rule.or;
export const createLogicExpect = <T>() => (k: keyof T & string) => Expect<T>(k);
export const createEvaluator = wrap;
