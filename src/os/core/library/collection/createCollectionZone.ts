...
import { resolveItemFallback } from "@/os/4-command/focus/focusStackOps";
import { OS_SELECTION_CLEAR } from "@/os/4-command/selection/selection";
import type { AppHandle } from "@/os/app/defineApp/types";
...
import {
  ...
  isEntityConfig,
  opsFromAccessor,
} from "@/os/core/library/collection/collectionZone.core";
import {
  findInTree,
  findParentOf,
  insertChild,
  removeFromTree,
  type TreeNode,
} from "@/os/core/library/collection/treeUtils";
...
>(
  ...
  const remove = zone.command(
    ...
    (ctx: { readonly state: S }, payload: { id: string }) => {
      ...
      const isNested =
        !isRoot && !!findInTree(allItems as TreeNode[], payload.id);
  ...
  const duplicate = zone.command(
    ...
    (ctx: { readonly state: S }, payload: { id: string }) => {
      ...
      const original = findInTree(items as TreeNode[], payload.id) as
        | T
        | undefined;
    ...
    (ctx: { readonly state: S }, payload: { afterId?: string }) => {
      ...
      const nextState = produce(ctx.state, (draft) => {
        ...
        const targetNode = targetId
          ...
          : undefined;
        const targetAccept = (
          targetNode as Record<string, unknown> | undefined
        ...
        for (let i = 0; i < clipItems.length; i++) {
          ...
          const itemType = (source as Record<string, unknown>)["type"] as
            | string
            | undefined;
  ...
  const move = zone.command(
    ...
    (
      ...
      if (payload.toParentId) {
        ...
        if (
          targetParent?.["accept"] &&
          !(targetParent["accept"] as string[]).includes(
  ...
  function collectionBindings(options?: {
    ...
    const guarded = <
      F extends (cursor: CursorArg) => BaseCommand | BaseCommand[],
    >(
      fn: F,
    ): F =>
      guard
        ? (((cursor: CursorArg) => (guard(cursor) ? fn(cursor) : [])) as F)
        : fn;
