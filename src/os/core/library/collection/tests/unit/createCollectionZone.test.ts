...
import type { AppHandle, ZoneHandle } from "@/os/app/defineApp/types";
import {
...
describe("createCollectionZone", () => {
  ...
  it("onCopy without selection should copy focused item only", () => {
    ...
    expect(result).toBeDefined();
    expect(result["state"]).toBe(mockState);
    expect((result["clipboardWrite"] as Record<string, unknown>)["text"]).toBe(
      "Item B",
    );
