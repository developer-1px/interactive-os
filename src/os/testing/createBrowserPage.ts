...
import { initialOSState } from "../core/schema/state/initial";
...
function createVisualEffects(
  ...
  interface TrackedStamp {
    el: Element;
    stampDiv: HTMLElement;
  }
  ...
  const bubbleTray = cursorEl.querySelector(
    ".testbot-bubble-tray",
  ) as HTMLElement;
      ...
      const inner = keys
        .map(
          (k, i) =>
            ...
            (i < keys.length - 1
              ? `<span class="testbot-keycap-plus">+</span>`
              : ""),
        )
        .join("");
...
export function createBrowserPage(
  ...
  const fx = headless
    ? createHeadlessEffects()
    : createVisualEffects(container, speed);
