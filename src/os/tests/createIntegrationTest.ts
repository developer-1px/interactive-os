 ...
 */
export function createIntegrationTest<S>(app: AppHandle<S>, initialStateOverride?: Partial<S>) {
    ...
    function resetZone(zoneId: string, overrides: Partial<AppState['os']['focus']['zones'][string]> = {}) {
        kernel.setState(produce((state: AppState) => {
    ...
    return {
        kernel,
        get state() { return testInstance.state; },
        get os() { return (kernel.getState() as any).os as AppState['os']; },
        resetZone,
