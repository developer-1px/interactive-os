...
import { FieldRegistry } from "@os/features/keyboard/registry/FieldRegistry.ts";
import type { KeyboardCategory, KeyboardIntent } from "../../../features/keyboard/types.ts";
...
function hasKeybinding(canonicalKey: string): boolean {
  const allBindings =
    useCommandEngineStore.getState().getAllKeybindings() ?? [];
