...
import { useInspectedElementStore } from "@os/inspector/InspectedElementStore";
import {
    Box,
    Code,
    Crosshair,
...
function PropValue({ value }: { value: unknown }) {
    if (typeof value === "string") {
        ...
        return (
            <span style={{ ...S.propValue, ...S.stringValue }}>"{value}"</span>
        );
    }
    ...
    if (typeof value === "object") {
        try {
            ...
            if (str.length > 60) return <span style={S.propValue}>{str.slice(0, 57)}…</span>;
...
function BoxModelDiagram({
    ...
    boxModel: NonNullable<ReturnType<typeof useInspectedElementStore.getState>["boxModel"]>;
}) {
    ...
    const contentW = Math.round(
        boxModel.width - paddingLeft - paddingRight - boxModel.borderLeft - boxModel.borderRight,
    );
    const contentH = Math.round(
        boxModel.height - paddingTop - paddingBottom - boxModel.borderTop - boxModel.borderBottom,
    );
        ...
        <div style={S.panel as any}>
                    ...
                    <div>
                        ...
                        <div style={S.row}>
                            <div style={{ display: "flex", alignItems: "center", gap: "6px", overflow: "hidden" }}>
                                <span style={{ color: "#6B7280", fontWeight: 600 }}>&lt;{tagName}&gt;</span>
                                {primitiveName && (
                                    <span
                                        style={{
                                            padding: "0 4px",
                            ...
                            <div style={{ display: "flex", alignItems: "center", gap: "6px", marginLeft: "auto", fontSize: "10px" }}>
                                {source ? (
                                    <>
                                        ...
                                        {source.loc !== undefined && source.loc > 200 && (
                                            <span style={{ color: "#EF4444", fontWeight: 700 }}>⚠️</span>
                                        )}
                                    </>
                                ) : (
                        ...
                        <div style={{ ...S.row as any, borderBottom: "none", color: "#666" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: "8px", width: "100%" }}>
                                ...
                                <span style={{ fontFamily: "monospace" }}>
                                    {boxModel ? `${Math.round(boxModel.width)}×${Math.round(boxModel.height)}` : "—"}
                                </span>

                                <span style={{ color: "#e5e5e5" }}>|</span>

                                {/* Radius */}
                                {boxModel && boxModel.borderRadius && boxModel.borderRadius !== "0px" ? (
                                    <span style={{ color: "#EC4899" }}>R: {boxModel.borderRadius}</span>
                                ) : (
                                ...
                                {boxModel && (boxModel.rowGap > 0 || boxModel.colGap > 0) ? (
                                    <span>Gap: {boxModel.rowGap === boxModel.colGap ? boxModel.rowGap : `${boxModel.rowGap}/${boxModel.colGap}`}</span>
                                ) : (
            ...
            <div style={S.section}>
                <div style={S.sectionHeader} onClick={() => toggleSection("os")}>
                    ...
                    OS Context
                    <span
                        style={{ marginLeft: "auto", fontSize: "9px", color: "#ccc" }}
                    >
                        {expandedSections.os ? "▾" : "▸"}
                    </span>
                </div>
                    ...
                    <div style={{ padding: "4px 0" }}>
                        {componentStack.length > 0 ? (
                            componentStack.map((name, i) => (
                                ...
                                >
                                    {i > 0 && (
                                        <span style={{ color: "#ddd", marginRight: "4px" }}>
                                            └
                                        </span>
                                    )}
                                    {name}
                            ...
                            <div style={{ padding: "8px 12px", color: "#ccc", fontSize: "10px", fontStyle: "italic" }}>
                                Stack unavailable
                            </div>
                        )}
                    </div>
