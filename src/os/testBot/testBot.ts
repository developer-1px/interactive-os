...
export function testBot(opts?: { speed?: number }): TestBot {
    ...
    const dryRun = async (): Promise<SuiteResult[]> => {
        ...
        for (const suite of suites) {
            ...
            try { await suite.fn(createMockActions(steps)); }
            catch { steps.push({ action: "error", detail: "Dry run error", passed: false }); }
            results.push({ name: suite.name, steps, passed: false, status: "planned" });
        }
        return results;
    };

    /** Execute a single suite, returns its result */
    const executeSuite = async (suite: typeof suites[0], suiteIndex: number, onStep?: OnStep): Promise<SuiteResult> => {
        const steps: StepResult[] = [];
        const actions = createActions(steps, { cursor: cursor!, speed, onStep, suiteIndex });

        try { await suite.fn(actions); }
        catch (e) {
            if (!(e instanceof Error) || e.name !== "BotError") {
                steps.push({ action: "error", detail: String(e), passed: false, error: String(e) });
    ...
    const runAll = async (onProgress?: OnProgress, onStep?: OnStep): Promise<SuiteResult[]> => {
        cursor = createCursor();
        aborted = false;
        const results: SuiteResult[] = [];
    ...
    const runSuite = async (index: number, onStep?: OnStep): Promise<SuiteResult> => {
        const suite = suites[index];
        if (!suite) throw new Error(`Suite index ${index} out of range (${suites.length} suites)`);
    ...
    const destroy = () => { aborted = true; cursor?.destroy(); cursor = null; };
