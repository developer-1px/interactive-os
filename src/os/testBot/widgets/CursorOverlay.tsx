...
function BubbleItem({ bubble }: { bubble: CursorBubble }) {
    ...
    const renderContent = () => {
        if (bubble.label === "Click") {
            return <><IconClick /><span style={{ marginLeft: 4 }}>Click</span></>;
        }
    ...
    return (
        <div className={`testbot-bubble variant-${bubble.variant}${fading ? " fading" : ""}`}>
            {renderContent()}
        </div>
    );
}
        ...
        <>
            ...
            >
                ...
                <div
                    ...
                    style={offScreen ? { transform: `rotate(${offScreenRotation}deg)` } : undefined}
                >
