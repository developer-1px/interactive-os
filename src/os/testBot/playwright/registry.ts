...
export function pushTest(name: string, fn: Function) {
    const entry: TestEntry = { type: "test", name, fn, sourceFile: currentLoadingFile };
    if (currentSuite) {
...
export function pushDescribe(name: string, fn: Function) {
    const suite: TestEntry = { type: "describe", name, fn, children: [], sourceFile: currentLoadingFile };
