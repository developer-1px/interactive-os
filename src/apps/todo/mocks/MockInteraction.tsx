...
export const MockButton: React.FC<MockButtonProps> = ({
  ...
  const baseStyle =
    "flex items-center justify-center gap-2 rounded-md transition-all duration-200 font-medium text-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-offset-[#090A0C] disabled:opacity-50 disabled:pointer-events-none";
...
export const MockToggle: React.FC<MockToggleProps> = ({
  ...
  return (
    <label
      className={`flex items-center gap-3 cursor-pointer group ${disabled ? "opacity-50 pointer-events-none" : ""} ${className || ""}`}
    >
      <div className="relative">
        ...
        />
        <div
          className={`w-10 h-6 rounded-full transition-colors duration-200 border border-transparent ${checked ? "bg-indigo-600" : "bg-slate-700"}`}
        ></div>
        <div
          className={`absolute left-1 top-1 w-4 h-4 rounded-full bg-white transition-transform duration-200 shadow-sm ${checked ? "translate-x-4" : "translate-x-0"}`}
        ></div>
      </div>
      {label && (
