...
export const MockWindow: React.FC<MockWindowProps> = ({
  ...
  return (
    ...
    >
      {/* TitleBar */}
      <div
        className={`h-10 border-b border-white/5 flex items-center justify-between px-4 select-none ${isActive ? "bg-white/5" : "bg-transparent"}`}
      >
        <div className="flex items-center gap-2">
          <div className="flex gap-1.5 group">
            <div
              onClick={onClose}
              className="w-3 h-3 rounded-full bg-red-500/20 border border-red-500/50 hover:bg-red-500 transition-colors flex items-center justify-center cursor-pointer"
            >
              <X
                size={8}
            ...
            <div className="w-3 h-3 rounded-full bg-green-500/20 border border-green-500/50 hover:bg-green-500 transition-colors flex items-center justify-center cursor-pointer">
              <Square
                size={6}
                className="text-black opacity-0 group-hover:opacity-100 fill-current"
              />
            </div>
          </div>
        </div>
  ...
  if (label && orientation === "horizontal") {
    return (
      <div className={`flex items-center gap-3 w-full my-4 ${className || ""}`}>
        ...
        />
        <span className="text-[10px] bg-white/5 px-2 py-0.5 rounded-full text-slate-500 uppercase tracking-widest font-bold">
          {label}
        </span>
        <div
          className="h-px bg-white/10 flex-1"
