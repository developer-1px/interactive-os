...
function defineCommands(define: Parameters<Parameters<typeof createModule>[2]>[0]) {
    return {
        commands: {
                    ...
                    (payload: { focusId: number | string }) => ({
                        state: produce(ctx.state, (draft) => {
                            ...
                            const visibleIds = ctx.state.data.todoOrder.filter(
                                (id) => ctx.state.data.todos[id]?.categoryId === ctx.state.ui.selectedCategoryId,
                            );
            ...
            redoCommand: define.command(
                ...
                (ctx: { state: AppState }) =>
                    () => {
                        ...
                        const entry = ctx.state.history.future[ctx.state.history.future.length - 1]!;
                        const focusTarget = entry.focusedItemId
                            ? String(entry.focusedItemId)
                            : undefined;
                        return {
