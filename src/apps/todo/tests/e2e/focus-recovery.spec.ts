...
test.describe
  .serial("Todo App - Delete Focus Recovery", () => {
    ...
    test("Deleting middle item moves focus to next item (Item C)", async ({
      page,
    }) => {
      // 1. Focus Item B
      await page.getByRole("option", { name: "Item B" }).click();
      await expect(
        page.getByRole("option", { name: "Item B" }),
    ...
    test("Deleting last item moves focus to previous item (Item B)", async ({
      page,
    }) => {
      // 1. Focus Item C
