...
import { TodoApp, addTodo, confirmDeleteTodo, requestDeleteTodo, undoCommand } from "@apps/todo/app";
...
describe("Bulk Delete + Undo — Reproduction", () => {
    it("bulk delete 2 items → undo → should restore ALL 2 items", () => {
        ...
        console.log(`[SETUP] todoOrder: ${JSON.stringify(page.state.data.todoOrder)}`);
        ...
        console.log(`pendingDeleteIds: ${JSON.stringify(page.state.ui.pendingDeleteIds)}`);
        ...
        console.log(`  Actual todoOrder: ${JSON.stringify(page.state.data.todoOrder)}`);
