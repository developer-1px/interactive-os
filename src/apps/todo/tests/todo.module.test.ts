...
describe("CRUD", () => {
  ...
  test("deleteTodo removes item", () => {
    ...
    expect(app.select.visibleTodos().some((t) => t.text === "Delete me")).toBe(
      false,
    );
    vi.restoreAllMocks();
  });
...
describe("Ordering", () => {
  test("moveItemUp swaps positions", () => {
    ...
    expect(beforeIds.indexOf(secondId)).toBeGreaterThan(
      beforeIds.indexOf(firstId),
    );
...
describe("Clipboard", () => {
  test("duplicateTodo creates copy", () => {
    ...
    const copies = app.select
      .visibleTodos()
      .filter((t) => t.text === "Original");
...
describe("Module Instance", () => {
  ...
  test("reset() returns to initial state", () => {
    ...
    expect(
      app.select.visibleTodos().some((t) => t.text === "Will be gone"),
    ).toBe(true);
