...
import { useDragState } from "@os/6-project/accessors/useDragState";
...
function TaskItemEditor({ todoId }: { todoId: string }) {
  const text = TodoApp.useComputed((s) => s.data.todos[todoId]?.text ?? "");
...
export function TaskItem({ todoId }: TaskItemProps) {
  ...
  const isDropTarget =
    ...
    dragState.overItemId === todoId &&
    dragState.dragItemId !== todoId;
  ...
  return (
    <>
      ...
      {dropPosition === "before" && (
        <div className="h-0.5 bg-indigo-500 rounded-full mx-4 -my-1 relative z-20">
          <div className="absolute -left-1 -top-1 w-2.5 h-2.5 rounded-full bg-indigo-500" />
        </div>
      )}
      <TodoList.Item
        id={String(todo.id)}
