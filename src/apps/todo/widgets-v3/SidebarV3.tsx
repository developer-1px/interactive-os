/**
 * SidebarV3 — defineApp/createWidget version.
 *
 * Before (v2): 5 lines of manual Zone bindings (onAction, onMoveUp, onMoveDown, onUndo, onRedo)
 * After (v3): <TodoSidebar.Zone> with 0 bindings
 */

import { TodoApp, TodoSidebar } from "@apps/todo/v3/app";
        ...
        <div className="w-72 flex flex-col h-full bg-[#FCFCFD] border-r border-slate-100 relative overflow-hidden">
            <div className="p-6 pb-2 z-10">
                <h1 className="font-bold text-slate-900 text-lg tracking-tight flex items-center gap-3">
                    ...
                    <span className="opacity-100">Todo v3</span>
                </h1>
            </div>
                ...
                {(state.data.categoryOrder ?? []).map((categoryId) => {
                    ...
                    return (
                        <TodoSidebar.Item key={category.id} id={category.id} asChild>
                            <OS.Trigger onPress={TodoSidebar.commands.selectCategory({ id: category.id })}>
                                <div
