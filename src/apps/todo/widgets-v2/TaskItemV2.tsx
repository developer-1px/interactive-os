/**
 * TaskItemV2 — TodoModule-based version.
 * Single import: TodoModule. No distributed command imports.
 */
...
export function TaskItemV2({ todo, isEditing, editDraft }: TaskItemProps) {
    ...
    return (
        ...
        >
            {/* Drag Handle */}
            <div
                className={`mt-1 -ml-1 text-slate-300 cursor-grab active:cursor-grabbing transition-opacity ${isHovered ? "opacity-100" : "opacity-0"}`}
            >
                <GripVertical size={16} />
            </div>

            {/* Checkbox Trigger */}
            <OS.Trigger onPress={cmds.toggleTodo({ id: todo.id })}>
                <div
                    className={`
          ...
          `}
            ...
            <div className="flex-1 min-w-0 pt-0.5">
                {isEditing ? (
                    <OS.Field
                        ...
                        value={editDraft}
                        autoFocus
                        onChange={cmds.syncEditDraft}
                        onSubmit={cmds.updateTodoText}
                        onCancel={cmds.cancelEdit()}
            ...
            {/* Quick Actions */}
            <div
                className={`flex items-center gap-1 transition-all ${isEditing || isHovered ? "opacity-100 translate-x-0" : "opacity-0 translate-x-2"}`}
            >
                {isEditing ? (
                    ...
                    <>
                        <OS.Trigger onPress={cmds.startEdit({ id: todo.id })}>
                            <button type="button" className="p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-slate-100 rounded-lg transition-colors" title="Edit (Enter)">
                        ...
                        <OS.Trigger onPress={cmds.moveItemUp({ focusId: todo.id })}>
                            <button type="button" className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors" title="Move Up (Cmd+Up)">
                                <ArrowUp size={14} />
                            </button>
                        </OS.Trigger>
                        <OS.Trigger onPress={cmds.moveItemDown({ focusId: todo.id })}>
                            <button type="button" className="p-1.5 text-slate-400 hover:text-slate-700 hover:bg-slate-100 rounded-lg transition-colors" title="Move Down (Cmd+Down)">
                                <ArrowDown size={14} />
                            </button>
                        </OS.Trigger>
                        ...
                        <OS.Trigger onPress={cmds.deleteTodo({ id: todo.id })}>
