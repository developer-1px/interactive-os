/**
 * SidebarV2 — TodoModule-based version.
 * Single import: TodoModule.
 */
...
export function SidebarV2() {
  ...
  return (
    <OS.Zone
      ...
      onAction={cmds.selectCategory({ id: OS.FOCUS as any })}
      onMoveUp={cmds.moveCategoryUp()}
      onMoveDown={cmds.moveCategoryDown()}
    ...
    <div className="w-72 flex flex-col h-full bg-[#FCFCFD] border-r border-slate-100 relative overflow-hidden">
      <div className="p-6 pb-2 z-10">
        <h1 className="font-bold text-slate-900 text-lg tracking-tight flex items-center gap-3">
          ...
          <span className="opacity-100">Todo v2</span>
        </h1>
      </div>
        ...
        {(state.data.categoryOrder ?? []).map((categoryId) => {
          ...
          return (
            <OS.Item key={category.id} id={category.id} asChild>
              <OS.Trigger onPress={cmds.selectCategory({ id: category.id })}>
                <div
