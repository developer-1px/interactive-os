...
export function ColumnView({ column, cards, state }: ColumnViewProps) {
    ...
    return (
        <div
            className={`
                w-80 flex-shrink-0 flex flex-col max-h-full rounded-2xl border transition-all
                ${column.collapsed
                    ? "w-14 cursor-pointer"
            ...
            {column.collapsed ? (
                ...
                <>
                    ...
                    <Zone
                        ...
                        className="flex-1 p-2.5 space-y-2.5 overflow-y-auto custom-scrollbar min-h-[80px] outline-none"
                    >
                        {cards.map((card) => (
                            <KanbanCard
                                key={card.id}
                        ...
                        {cards.length === 0 && !draftText && (
                            <div className="h-20 flex flex-col items-center justify-center text-slate-300 gap-1.5 pointer-events-none">
                                <span className="text-[10px] font-bold uppercase tracking-widest opacity-60">No Cards</span>
                            </div>
                        )}
                    </Zone>
                </>
