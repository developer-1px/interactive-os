...
import { SetPriority, SetDueDate, SetAssignee } from "@apps/kanban/features/commands/card";
    ...
    return (
        <div className="fixed inset-0 z-50 flex justify-end">
            ...
            <Zone
                id="card-detail"
                role="dialog"
                className="relative w-[480px] max-w-full bg-white shadow-2xl border-l border-slate-200 flex flex-col overflow-hidden animate-slide-in-right"
            >
                {/* Header */}
                <div className="flex items-center justify-between p-5 border-b border-slate-100 bg-slate-50/50">
                    <h2 className="text-sm font-bold text-slate-500 uppercase tracking-wider">Card Detail</h2>
                    <button
                        onClick={() => dispatch(CloseCardDetail({}))}
                ...
                <div className="flex-1 overflow-y-auto p-5 space-y-5">
                    ...
                    <div>
                        ...
                        <input
                            value={card.title}
                            onChange={(e) => dispatch(UpdateCardTitle({ id: card.id, title: e.target.value }))}
                    ...
                    <div>
                        ...
                        <textarea
                            ...
                            onChange={(e) => dispatch(UpdateCardDescription({ id: card.id, description: e.target.value }))}
                    ...
                    <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {PRIORITIES.map(({ value, label, color }) => (
                                <button
                                    key={value}
                                    onClick={() => dispatch(SetPriority({ id: card.id, priority: value }))}
                                    className={`
                        ...
                        <div className="flex flex-wrap gap-2">
                            {allLabels.map((label) => {
                                ...
                                return (
                                    <button
                                        key={label.id}
                                        onClick={() => dispatch(ToggleLabel({ id: card.id, labelId: label.id }))}
                                        className={`
                                            px-3 py-1.5 rounded-lg text-xs font-bold transition-all border
                                            ${isActive
                    ...
                    <div>
                        ...
                        <input
                            value={card.assignee || ""}
                            onChange={(e) => dispatch(SetAssignee({ id: card.id, assignee: e.target.value || null }))}
                    ...
                    <div>
                        ...
                        <input
                            type="date"
                            value={card.dueDate || ""}
                            onChange={(e) => dispatch(SetDueDate({ id: card.id, dueDate: e.target.value || null }))}
                    ...
                    <div className="pt-3 border-t border-slate-100">
                        <div className="flex items-center gap-1.5 text-[10px] text-slate-300 font-medium">
                            <Clock size={10} />
                            Created {new Date(card.createdAt).toLocaleDateString("en", { year: "numeric", month: "short", day: "numeric" })}
                        </div>
                    </div>
                </div>
                ...
                <div className="p-3 border-t border-slate-100 bg-slate-50/50 flex items-center justify-between">
                    <span className="text-[10px] text-slate-300 font-medium">Tab to navigate fields</span>
                    <span className="text-[10px] text-slate-300 font-medium">
                        <kbd className="px-1.5 py-0.5 bg-white border border-slate-200 rounded text-[9px] font-bold">Esc</kbd> to close
                    </span>
                </div>
            </Zone>
        </div>
