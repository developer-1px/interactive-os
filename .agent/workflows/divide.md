...
- 정답 있는 건 **단위 테스트로 증명하고** 고민 없이 실행
- 정답 없는 건 코드로 해결하려 하지 말고 나누거나 질문
- 코드 수정 전에 "정답인가?" 판단 — 수정 중에 판단하면 이미 늦다
- **판단 재분류**: 수정 중 Open이 Known으로 밝혀지면, 보고서를 업데이트하고 바로 실행
