---
description: 프로젝트 완료 시 지식을 2-area 및 rules로 환류하고, 잔여 산출물은 4-archive/연도/월/주차로 매장한다.
---

## /archive — 프로젝트 완료 & 지식 환류

> **목적**: 프로젝트 완료 시 지식을 소멸시키지 않고 살아있는 문서(2-area, rules)로 환류한다.
...
```
살아있는 문서 (덮어쓰기, 지형 기반):
  2-area/   ← 공식 지식의 인지 지도 (소스코드 토폴로지와 동형)
...
**살아있는 문서**는 소스코드 토폴로지에 기반하여 안 늙는다 (2-area/, rules.md).
**죽은 문서**는 시점에 바인딩되어 노화한다 — 분류할 필요 없이 시간순으로 매장한다.

`/archive`의 역할: **프로젝트의 지식을 2-area 및 rules로 추출 → 나머지는 4-archive/주차에 매장.**
...
| 판단 기준 | 행선지 | 예시 |
|-----------|--------|------|
| **시점 독립 원칙/스펙** | `2-area/` 갱신 또는 `rules.md` 추가 | 커맨드 동작 스펙, 아키텍처 원칙 |
...
- 프로젝트에서 확정된 스펙/동작이 있으면 → `2-area/` 해당 섹션 **덮어쓰기**
