...
- `/tdd`는 PRD의 시나리오를 테스트 케이스로 번역한다.
- `/review`는 코드가 PRD 시나리오를 충족하는지 검증한다.
- `/issue`의 버그는 "PRD 대비 이탈(deviation from PRD)"로 정의된다.
...
- **테스트 가능**: 각 Scenario를 그대로 `/tdd`에서 테스트 케이스로 번역할 수 있어야 한다.
...
- ✅ 승인 → BOARD.md 태스크를 PRD 기준으로 정렬, 다음 단계(proposal 또는 /tdd)로 진행.
