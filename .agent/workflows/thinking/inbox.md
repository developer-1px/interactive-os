...
2. **Draft Report Content**
   - Structure the report with the following sections (adapt as needed):
     ...
     - **메타 테이블**: 제목 바로 아래에 다음 항목을 표로 정리한다.
        ...
        - `내(AI)가 추정한 의도`: 3줄로 작성한다.
          1. **경위** — 이 요청이 나오게 된 맥락/상황 한줄.
          2. **표면** — 사용자가 명시적으로 요구한 것 한줄.
          3. **의도** — AI가 추론한 진짜 목적 한줄.
     ...
     - **6. 열린 질문 (Complex Questions)**: 사용자의 의사결정이 필요한 항목을 번호 목록으로 제시한다. Clear/Complicated 문제는 여기에 넣지 않고 제안에서 바로 답을 제시한다.
      - **3줄 요약**: blockquote(`>`)로 감싸서 3줄로 보고서 핵심을 요약한다. 문서 목록에서 훑어볼 때 이것만 보고 내용을 떠올릴 수 있어야 한다.

3. **저장 위치 결정 (프로젝트 컨텍스트 라우팅)**
   - `docs/STATUS.md`를 읽어 현재 Active Focus 프로젝트를 확인한다.
   - **현재 대화에서 프로젝트 진행 중**: 해당 `docs/1-project/[name]/notes/`에 저장. 대화 맥락이 곧 프로젝트 맥락이므로 별도 관련성 판단 불필요.
   - **프로젝트 진행 중이 아닌 경우** (독립적 질문, 새 주제):
     - Focus 프로젝트와 매칭 시도 → 매칭됨 → 해당 프로젝트 `notes/`에 저장
