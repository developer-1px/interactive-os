...
5. **리포트 작성**
   ...
   - **심각도 × 의도 2축 분류**:
     - 심각도:
       ...
       - 🔵 **개선 제안**: 더 나은 패턴
     - 의도:
       - `[Blocker]` — 반드시 수정. 이대로 커밋 불가
       - `[Suggest]` — 개선 제안. 수정하면 좋지만 필수 아님
       - `[Question]` — 이해를 위한 질문. 코드 의도 확인
       - `[Nitpick]` — 사소한 스타일. 무시해도 됨
       - `[Praise]` — 잘한 부분. 패턴 학습 목적
     - 조합 예시: `🔴 Blocker: useEffect로 상태 동기화 — 커널 단방향 흐름 위반`
     - 조합 예시: `🟡 Suggest: 변수명 items → cartItems로 명확화`
     - 조합 예시: `🔵 Praise: Zone 스코핑 패턴 정확히 적용됨`
