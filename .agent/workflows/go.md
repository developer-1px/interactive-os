...
| M9 | `/verify` 통과 + Unresolved > 0 | → Now 승격 | Unresolved를 Now로 올리고 #0 루프 |
| M10 | `/verify` 통과 + Unresolved == 0 | → 🤖 `/qa --meta` agent | **Meta QA** (아래 §Meta QA 참조) |
| M11 | `/qa --meta` FAIL | → 수정 → 재의뢰 | 리포트 기반 수정 후 M10 루프 |
| M12 | `/qa --meta` ALL PASS | → `/retrospect` → `/archive` | 종료 |
