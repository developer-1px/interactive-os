...
2. 기존 구현이 있는지 확인:
   ...
   - `src/os/4-command/tests/apg/{pattern}.apg.test.ts` 존재?
   - `src/os/testing/scripts.ts`에 TestBot 스크립트 존재?
...
**테스트 파일**: `src/os/4-command/tests/apg/{pattern}.apg.test.ts`
...
```bash
source ~/.nvm/nvm.sh && nvm use && npx vitest run src/os/4-command/tests 2>&1 | tail -6
```
