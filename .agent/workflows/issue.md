...
#### 5. 수정 계획 → /reflect 재귀 루프

계획과 점검을 **수렴할 때까지** 반복한다:
...
while (true) {
  ...
  plan = 수정 계획 수립
  gaps = /reflect 점검
  if (gaps === 0) break    // 수렴: 계획이 본질이고 빈틈 없음
  if (round >= 3) break    // 최대 3라운드
  // gaps 발견 → 계획 수정 후 다시 reflect
}
```
...
Gap 발견 시 계획을 수정하고 다시 reflect. 미발견 시 수렴.
