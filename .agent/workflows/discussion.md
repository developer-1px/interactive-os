...
1. **매 턴마다 3단계를 수행한다**:
   ...
   - **Gap 질문**: "이 주장이 성립하려면 어떤 전제가 빠져 있는가?"를 기준으로 1~2개 질문
...
2. **Clear/Complicated는 즉답, Complex에만 Gap 질문** (Cynefin Framework, `rules.md` 참조):
   ...
   - **자가 점검**: "이 질문에 정답이 존재하는가?" → Yes면 Clear/Complicated다. Gap 질문 없이 즉답한다.

3. **How가 아닌 Why를 추적한다**:
   - 사용자가 **How(구체적 구현 방법)**를 제시해도 바로 실행하지 않는다.
   - 그 How 뒤에 숨겨진 **Why/Intent를 먼저 추출**한다.
   - Intent를 이해한 뒤, **기존 수단으로 해결 가능한지 먼저 확인**한다. 가능하면 새 개념을 추가하지 않는다.
   - **자가 점검**: "이 방안은 새 개념/레이어를 도입하는가?" → Yes면, 새 개념 없이 동일 Intent를 달성하는 대안을 1개 이상 제시한 뒤 비교한다.
...
- 원문을 최대한 그대로 인용하되, **전환점(Intent 변경, 새 Warrant 발견)이 포함된 턴만 선별**하여 압축
- 형식: `**🧑 사용자**: ...` / `**🤖 AI**: ...` 로 턴을 구분
- 전환점마다 `---` 구분선으로 단락 분리
- 마지막에 **한 줄 요약**으로 마무리
