...
> ⚠️ **필수.** headless-test-guide.md §7 참조.

1. 해당 앱의 기존 `testbot-*.ts` 파일이 있으면 → 스크립트를 **추가**
2. 없으면 → `src/[app-path]/testbot-[app].ts` 생성 (zones, group, scripts export)
3. 재현 스크립트를 TestScript 형식(`{ name, zone, run }`)으로 작성
4. vitest용 `.test.ts`는 `tests/headless/apps/[app-name]/`에 위치
...
- [ ] `.test.ts` 파일 존재
- [ ] TestBot에 등록됨 (`testbot-*.ts`에 스크립트 추가/생성)
