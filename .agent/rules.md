...
13. **기억이 아닌 산출물로 강제한다.** LLM은 규칙을 읽어도 goal fixation으로 건너뛴다. 각 단계의 산출물(설계 문서, 체크리스트)이 다음 단계의 입력이 되도록 하여, 산출물 없이는 다음 단계로 갈 수 없게 한다. 코드를 수정하기 전에 설계 문서가 존재해야 한다. 문서가 없으면 코드를 수정할 자격이 없다. → `/issue` D4, `/go` 모든 단계 통과
