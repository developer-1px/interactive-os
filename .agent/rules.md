...
### 4. 실패 원인 3분류 — 우회 코드 금지

커밋·빌드·테스트가 실패하면 **에러 메시지만 읽고 원인을 분류**한다. 파일을 다시 읽지 않는다.

| 분류 | 예시 | 행동 |
|------|------|------|
| **A. 환경** | lint-staged stash 복원, hook 충돌, tooling 오류 | 환경 해결 후 같은 코드 재시도 |
| **B. 실수** | typo, 잘못된 API 호출, 누락된 import | 에러 지점만 수정 후 재시도 |
| **C. 설계** | 접근 자체가 틀림, 타입 구조가 안 맞음 | 방향 변경 허용 |

- ⛔ **분류 없이 우회 코드 작성 금지.** 우회는 항상 새 이름(반창고 식별자)을 만든다. 새 이름이 glossary에 들어갈 자격이 없으면 그건 반창고다.
- A·B면 같은 방향을 유지한다. **C일 때만 방향을 변경**한다.
