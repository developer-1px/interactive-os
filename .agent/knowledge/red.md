...
| **거짓 GREEN** | OS-only 테스트로 앱 기능 검증 → 커널만 검증 → browser에서 전부 실패. 8 tests GREEN + audit PASS + doubt PASS 했지만 ArrowDown/Escape/overlay 모두 동작 안 함 | 앱 테스트 = `createPage(app, Component)` + click/press만. (선례: dropdown-dismiss 2026-02) |
