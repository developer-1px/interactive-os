...
| 0.5 | `/issue` 완료 (standalone issue, BOARD 없음) | → #7 | issue도 audit→doubt→retrospect→archive를 거친다. **BOARD 유무와 무관하게 #7로 직행.** |
...
| 7 | 모든 Now Done (또는 issue 완료) → `/audit` 미실행 | → `/audit` | **필수 완료 게이트.** 프로젝트·issue 무관 전체 적용 |
...
| 1 | `/retrospect` | KPT 회고 |
| 2 | `/archive` | 프로젝트 매장 |
| 3 | `rm -f /tmp/.go-pipeline-active` | `/auto` 마커 해제 (있을 때만) |
