...
│   │   → 근거: "[표준/규칙] 에 의거"
│   │   → ⛔ 단, OS 코드(src/os/) 수정이면 A2로. OS는 "바로 해"의 대상이 아니다.
...
│   ├─ C2. 해법이 엔트로피를 늘리고 있다
│   │   → 멈춰. 그건 해결이 아니라 악화다.
│   │   → 체크: "새로운 유일한 패턴을 추가하는가?" → Yes면 멈춤
│   │   → eslint-disable, as any, document.querySelector = 엔트로피 냄새
│   │   → "아직 올바른 추상화를 못 찾았다" → B2로 (설계 필요)
...
| 9 | "기능이 없으니 DOM으로 우회" | "기능이 없으니 OS에 새 메커니즘 설계" | → B2 분기로 |
