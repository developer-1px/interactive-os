...
1. **지식 로딩**
   - `.agent/knowledge/contract-checklist.md` §Config — 필수 OS 패턴, 검사 grep 패턴, 분류 기준
   - `.agent/knowledge/audit.md` §2 — 알려진 OS 갭 (재발견해도 🟡. 새 🔴로 분류하지 말 것)
   - `.agent/knowledge/audit.md` §3 — 정당한 예외 (이미 허용된 것은 무시)
   - `.agent/knowledge/audit.md` §5 — 판정 선례 (같은 패턴 재논쟁 방지)
...
> `.agent/knowledge/contract-checklist.md` §Config의 grep 패턴을 사용한다.
> 필수 OS 패턴, 금지 목록, 분류 기준이 모두 해당 토픽 파일에 있다.
> **tests 폴더는 제외.**
...
- 검사 패턴/금지 목록 변경 → `contract-checklist.md` §Config 갱신
