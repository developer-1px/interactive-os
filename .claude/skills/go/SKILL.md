...
| 19 | `/qa` ALL PASS | → `/retrospect` → Done 마킹 | 종료. `/archive`는 사용자 수동 호출 |
...
| M15 | `/qa --meta` ALL PASS | → `/retrospect` → Done 마킹 | 종료. `/archive`는 사용자 수동 호출 |
...
| QA 통과 | `✅ QA PASS` → `/retrospect` → `🟢 Done` (사용자가 `/archive` 수동 호출) |
