...
| 모드 | 트리거 | 실행 게이트 |
|------|--------|------------|
| **Code** (기본) | Heavy/Light 프로젝트 | Gate 1~6 전체 |
| **Meta** (`--meta`) | Meta 프로젝트 | Gate 1(tsc) + Gate 3(unit) 만 — regression 확인 |

Meta 모드에서는 **새 코드가 없으므로** lint, bind smoke, build, dev smoke를 스킵한다.
tsc와 unit test만 실행하여 **기존 코드의 regression이 없음**을 확인한다.
