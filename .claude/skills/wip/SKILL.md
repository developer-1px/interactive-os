...
턴 N:
  1. Cynefin 판정 (현재 상태)
  2. 도구 선택 (적응형)
  3. Skill tool로 도구 호출 (인라인 실행 금지)
  4. 결과 반영 → Cynefin 재판정
```

> **⛔ 도구는 반드시 Skill tool로 호출한다.** 도구 이름만 언급하고 직접 판단하는 것은 금지.
> 예: `/doubt` 선택 → `Skill tool: skill="doubt"` 호출. "doubt해보면 불필요하다"라고 자체 판단하는 것은 인라인 실행이며 금지.
> **⛔ 1턴 = 1스킬.** 한 턴에 여러 스킬을 호출하지 않는다.
