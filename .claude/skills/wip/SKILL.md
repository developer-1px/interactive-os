...
### 현재 Cynefin: [Complex/Complicated]

### Open Gaps (인간 입력 필요)

- [ ] Q1: [질문] — 이것이 해소되면 [어떤 분해가 가능해진다]
- [ ] Q2: [질문] — 이것이 해소되면 [어떤 충돌이 풀린다]
...

### 다음 /wip 시 시작점

[여기서부터 이어가면 된다. 어떤 도구를 먼저 돌리면 좋겠다는 제안]
```

### 환류 후

1. 커밋한다 (메시지: `docs(backlog): enrich [항목명] via /wip probe`)
2. 종료한다.
...
- **비가역 행동 없음**: `/wip`은 분석 + 문서 작성만 한다. 코드 수정, 프로젝트 삭제 등 금지.
- **N턴 hard limit**: 3턴 초과 불가. "한 턴만 더" 금지.
- **Clear 경로의 `/go` 위임**: `/wip` 자체가 코드를 수정하지 않는다. 실행은 반드시 `/go`를 통한다.
