---
description: 특정 인터랙션이 눈에 띄게 느릴 때 사용. 측정 없이 최적화하지 않으며, Before/After 수치를 반드시 확인한다.
---
...
```
① 구독 패턴 — 상태 구독(selector/computed)이 원시값(boolean/number)을 반환하는가?
   → string이나 객체를 반환하면 모든 구독자가 리렌더된다.
   → selector 안에서 === 비교하여 원시값을 반환해야 한다.
...
- 자가 점검: "이 수정은 증상 치료인가, 근본 치료인가?"

#### 5. 재측정 (코드 수정 후)

Step 2와 **동일한 측정**을 반복한다. Before → After 숫자를 비교한다.
