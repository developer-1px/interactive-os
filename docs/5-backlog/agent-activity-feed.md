...
- [x] ~~Q1: JSONL vs git-log~~ → **보완적 사용**. JSONL=실시간 tool calls (현행 유지), git-log=커밋 맥락 (추가)
- [x] ~~Q2: os-migration 블로커~~ → **독립 구현 가능** (T8은 Dialog overlay에만 영향)
- [ ] **Q3 (재정의): 기존 RecentSection 개선이면 충분한가?** — "별도 뷰"가 아니라 DocsSidebar 내 기존 "Agent Activity" 섹션에 커밋 맥락 + 세션 그루핑 추가로 완료 조건 충족 가능. 인간 동의 시 즉시 Clear → `/project` → `/go`
