...
> **구현 위치**: `packages/os-core/src/` (순수 로직), `packages/os-react/src/` (React 바인딩)

1. **전송/신호(Transport/Signal)**: 물리적 키 입력과 클릭. → `packages/os-core/src/1-listen/`, `packages/os-react/src/1-listen/`
2. **해결(Resolution)**: 키바인딩과 컨텍스트를 통해 신호를 커맨드에 매핑. → `packages/os-core/src/2-resolve/`
3. **컨텍스트 주입**: DOM 데이터를 순수 함수에 주입. → `packages/os-core/src/3-inject/`
4. **상태 엔진**: 커맨드를 처리하여 불변 상태를 생성하는 중앙 로직. → `packages/os-core/src/4-command/`
5. **프로젝션(View)**: 상태를 렌더링하고 인터랙션 프리미티브를 제공하는 React 레이어. → `packages/os-react/src/6-project/`
