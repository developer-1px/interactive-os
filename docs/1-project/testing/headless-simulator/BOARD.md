...
- [x] T1: `ZoneRegistry.bindElement(id, el)` ✅ (os-restructure Phase 2에서 구현. zoneRegistry.ts line 240)
- [x] T2: `ZoneCallbacks` 타입 ✅ (zoneContext.ts line 111에 정의됨)
- [x] T3: `register()` + senseKeyboard/resolveClick 추출 ✅ (os-restructure #3-#5)
- [ ] T4: Phase 1 deps 축소 — useMemo deps 확인 필요
- [ ] T5: `buildZoneEntry`를 ZoneRegistry로 이동 (또는 register에 흡수)
- [ ] T6: deprecated `FocusGroupContext` 분리
- [ ] T1-V: e2e GREEN 복구 확인
...
### Phase 7: TestPage Playwright Isomorphism
- [x] T20: WP-DATA-ATTR — `data-item-id` 제거 + `data-focus-item` → `data-item`. DOM 스캔은 `el.id` 사용. — tsc 0 | +0 regression | 30+ files ✅
- [x] T21: WP-LOCATOR — `#id` selector 파싱 + 6 assertion helpers + `inputValue()`. — tsc 0 | types+page+headless+browser ✅
- [x] T22: WP-FIELD-EDITING — `data-editing` attr + `toBeEditing()` assertion. — tsc 0 | compute+types ✅
