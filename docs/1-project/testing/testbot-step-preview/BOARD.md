...
|---|------|----|--------|----------|
| T1 | createDryRunPage() 구현 — action 기록용 mock Page | BrowserStep[] 반환 | ⬜ | |
| T2 | initSuites 시 dry-run 실행하여 steps 사전 추출 | planned 상태의 suites에 steps 존재 | ⬜ | |
| T3 | TestBotPanel에서 planned + steps 있으면 step preview 표시 | "Ready to run" 대신 step 목록 | ⬜ | |
| T4 | E2E 검증 — /todo에서 planned suites에 step preview 노출 | Playwright test PASS | ⬜ | |
