...
### T2. Item onActivate -- dead prop 정리 [Hard error]
- **위치**: `packages/os-react/src/6-project/Item.tsx:47`
- **Before**: prop 존재하지만 ZoneRegistry.setItemCallback 안 함 (TriggerBase 경유로만 동작)
- **After**: ItemProps에서 onActivate 제거. TriggerBase 내부 경로는 internal prop으로 유지
- **의존**: T1 완료

### T5. TriggerBinding -- 미매칭 item lazy 경고 [Dev warning]
- **위치**: `packages/os-core/src/4-command/activate/activate.ts:39`
- **Before**: getItemCallback 있는데 item이 zone에 없어도 무시
- **After**: OS_ACTIVATE dispatch 시 callback은 있는데 item이 zone items에 없으면 console.warn
