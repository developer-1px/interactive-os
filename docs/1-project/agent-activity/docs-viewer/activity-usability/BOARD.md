...
- [ ] T1: `collectAgentActivity()` dedup 제거 + limit 100 — 크기: S, 의존: —
- [ ] T2: `AgentRecentFile.dir` 추가 + `getLatestRead()` + `getWrittenFilesBySession()` 신규 — 크기: S, 의존: →T1
- [ ] T3: `isGrouped` 상태 + `TOGGLE_GROUPING` + trigger 제거 — 크기: S, 의존: —
- [ ] T4: `RecentSection` → `AgentActivitySection` 전면 교체 (Read 인디케이터 + Write 세션 그룹 + dir 2단 + 활동 감지) — 크기: M, 의존: →T2, →T3
