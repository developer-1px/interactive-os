...
52개 스킬 중 **적응형 19개**의 description을 "무엇을 하는가" → "언제 호출되어야 하는가"로 재작성하고, Claude Code Skills 2.0의 eval 시스템으로 트리거 정확도를 측정·최적화한다.
...
- `/go`의 모호함 프로토콜이 `conflict → blueprint → divide` 3개로 하드코딩됨. 19개 적응형 도구 중 3개만 사용
...
| **적응형 도구** | 19 | ✅ **이 프로젝트 대상** | /why, /diagnose, /conflict, /blueprint, /naming |
| **인프라/유틸리티** | 17 | ❌ 독립 유틸 | /explain, /knowledge, /status, /mermaid |

## How — 접근

1. 적응형 19개 스킬의 SKILL.md description을 **트리거 조건 중심**으로 재작성
