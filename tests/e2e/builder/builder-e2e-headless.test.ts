...
describe("Builder Sidebar — Playwright-style locator", () => {
  it("sidebar vertical navigation with locator", () => {
    ...
    expect(
      page.locator("ge-hero").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
  ...
  it("sidebar full sweep: top to bottom", () => {
    ...
    for (let i = 0; i < expected.length; i++) {
      ...
      expect(
        page.locator(expected[i]!).toHaveAttribute("aria-current", "true"),
      ).toBe(true);
  ...
  it("sidebar selection: click selects, Cmd+Click toggles", () => {
    ...
    expect(page.locator("ge-hero").toHaveAttribute("aria-selected", true)).toBe(
      true,
    );
    ...
    expect(
      page.locator("ge-tab-nav").toHaveAttribute("aria-selected", true),
    ).toBe(true);
  });
  ...
  it("sidebar zone container attributes", () => {
    ...
    expect(
      page.locator("sidebar").toHaveAttribute("aria-current", "true"),
    ).toBe(true);
  });
});
...
describe("Builder Canvas — spatial navigation with mock rects", () => {
  it("canvas focus with mock layout", () => {
    ...
    console.log(
      `Canvas ArrowDown: focused = ${focusedAfter} (expected: no movement without rects)`,
    );
  });
});
